...
name: web-ui-test
version: 0.1.0
description: Test the ThinClaw web UI using the Claude for Chrome browser extension.
activation:
  keywords:
    - test web ui
    - test the ui
...
Use this skill when manually testing the ThinClaw web gateway UI via the Claude for Chrome browser extension.

## Prerequisites

- ThinClaw must be running with `GATEWAY_ENABLED=true`
...
Wait for "Agent thinclaw ready and listening" in the logs before proceeding.
...
```bash
rm -rf ~/.thinclaw/installed_skills/<skill-name>
```
