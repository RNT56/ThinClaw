...
async function main() {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (key.startsWith('OPENCLAW_') || key.startsWith('MOLTBOT_') || key.startsWith('CLAWDBOT_')) {
            console.log(`[openclaw-wrapper] Env: ${key}=${value}`);
        }
    }
    ...
    if (!openclawDir) {
        ...
        try {
            ...
            console.error('[openclaw-wrapper] Could not find openclaw package');
            console.error('[openclaw-wrapper] Error:', e.message);
            process.exit(1);
        }
    }
