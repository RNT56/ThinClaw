...
// A dummy reporter that drops events, since Moltbot doesn't consume the XML stream yet.
// In the future, we can forward these events to Moltbot via IPC if needed.
...
/// Handles incoming RPC requests from Moltbot (OpenClaw)
