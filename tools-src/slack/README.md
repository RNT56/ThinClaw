...
A standalone WASM component that provides Slack integration for ThinClaw. This serves as both a functional tool and a template for building custom WASM tools.
...
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.thinclaw/tools/slack.wasm
cp slack.capabilities.json ~/.thinclaw/tools/
```
...
```bash
thinclaw secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
thinclaw secret set slack_bot_token "xoxb-..."
```
