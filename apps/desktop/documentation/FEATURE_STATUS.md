...
| Desktop security baseline | Main and Spotlight windows have isolated least-privilege Tauri capabilities; renderer file reveal crosses a canonicalized Rust allow-root boundary; npm is audit-clean and the enabled Rust graph passes with no advisory ignores after the Tauri/ORT/OpenDAL refresh. | `security-and-dependencies.md`, `capabilities/*.json`, `deny.toml` |
| RAG and embeddings | Reranker files are revision/checksum verified with bounded inference and fail-soft ordering; provider outputs and asymmetric retrieval tasks are validated; cloud selections restore at startup; live local output drives safe vector-dimension migration while FTS stays available. | `rag-inference-compatibility.md`, `reranker.rs`, `inference/embedding/`, `rag.rs` |
