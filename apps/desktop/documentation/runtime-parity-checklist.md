...
| Channels | wired | Tauri local channel is first-class. Slack and Telegram settings are preserved. Remote mode maps gateway `channel_setup` status into desktop channel status entries, including Gmail and Apple Mail availability. Gmail status, Apple Mail settings, and pairing APIs now route through ThinClaw DB/gateway APIs; remote Gmail OAuth returns an explicit gateway-host unavailable response instead of running local OAuth. DM Pairing lists the five adapters that enforce shared pairing codes (Telegram, Slack, Discord, WhatsApp, Signal) and describes their credential setup without offering unsupported browser-login flows. |
