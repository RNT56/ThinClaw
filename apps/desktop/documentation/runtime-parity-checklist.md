...
| Channels | tested | Tauri local channel is first-class. Local and remote Channel Config paths validate the same schemas. Signal, Discord, iMessage, Nostr, Apple Mail, and BlueBubbles preserve current non-secret values; installed WASM capabilities map setup credentials to encrypted-store-only password fields; native Matrix/voice/APNs/browser-push expose honest host-managed instructions. Gmail status/settings and pairing APIs route through ThinClaw DB/gateway APIs; remote Gmail OAuth is explicitly host-unavailable. DM Pairing covers every adapter that enforces shared pairing codes (Telegram, Slack, Discord, WhatsApp, Signal). |
...
| Fleet | tested | The embedded runtime and every saved remote profile form the fleet model. Remote nodes use authenticated `/api/gateway/status` rather than an unauthenticated socket handshake; task assignment routes to the selected gateway; broadcast sends once per node and returns per-node delivery/error receipts. Profile credentials are never serialized into fleet status. |
