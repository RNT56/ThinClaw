...
| Channels | gated | Tauri local channel is first-class. Channel Config loads current non-secret schema values only; password values never cross IPC to React. Secret editing is deliberately unavailable until an encrypted channel-secret binding exists, rather than presenting a field the backend cannot honor. Native Matrix/voice/APNs/browser-push expose host-managed instructions. Gmail status/settings and pairing APIs route through ThinClaw DB/gateway APIs; remote Gmail OAuth is explicitly host-unavailable. DM Pairing covers every adapter that enforces shared pairing codes (Telegram, Slack, Discord, WhatsApp, Signal). |
...
| Fleet | tested | The embedded runtime and every saved remote profile form the fleet model. Remote nodes use authenticated `/api/gateway/status` rather than an unauthenticated socket handshake. The Desktop surface presents live profile status and broadcast delivery/error receipts only; targeted task assignment and abort are explicitly unavailable until a selected-profile dispatch contract exists. Profile credentials are never serialized into fleet status. |
...
| Learning | fixture-tested | Learning status/history/candidates/outcomes/proposals/reviews/rollbacks are visible. Learning Review also configures, activates, deactivates, and probes all eight external-memory adapters without accepting raw API-key values. Provider mutation is local-only and returns a typed gate in remote mode; health remains local/remote. Other mutation availability is tied to DB/runtime support and visible gate reasons. |
...
| Channel config | gated | `Channel::config_schema()` + DTOs; read commands + `thinclaw_channel_config_submit` (LocalOnly; WASM live, native restart-required); Signal/Discord impls + Channel Config panel. The schema response returns current non-secret values only, and secret binding is visibly unavailable pending encrypted-store support. |
