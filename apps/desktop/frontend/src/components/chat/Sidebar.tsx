import { AnimatePresence } from 'framer-motion';
import { useRef } from 'react';
import { cn } from '../../lib/utils';
import { useChatLayout } from './ChatProvider';
import { ModeNavigator } from '../navigation/ModeNavigator';
import { ChatSidebar } from './sidebars/ChatSidebar';
import { ThinClawSidebarSlice } from './sidebars/ThinClawSidebarSlice';
import { ImagineSidebarSlice } from './sidebars/ImagineSidebarSlice';
import { SettingsSidebarSlice } from './sidebars/SettingsSidebarSlice';

export function Sidebar() {
    const sidebarRef = useRef<HTMLElement>(null);
    const {
        sidebarOpen,
        setSidebarOpen,
        activeTab,
        appMode,
        isThinClawMode,
        isImagineMode,
        isSettingsMode,
        thinclawGatewayRunning,
        setActiveTab,
        openCommandPalette,
    } = useChatLayout();

    return (
        <aside
            ref={sidebarRef}
            aria-label="ThinClaw navigation"
            data-testid="app-sidebar"
            className={cn(
                "border-r border-border bg-card/50 backdrop-blur-sm flex flex-col transition-all duration-300 relative z-20 overflow-hidden h-full",
                sidebarOpen ? "w-64 p-4" : "w-16 p-2"
            )}
            onMouseEnter={() => setSidebarOpen(true)}
            onMouseLeave={() => {
                if (!sidebarRef.current?.contains(document.activeElement)) setSidebarOpen(false);
            }}
            onDragEnter={() => setSidebarOpen(true)}
            onDragOver={(e) => e.preventDefault()}
            onFocusCapture={() => setSidebarOpen(true)}
            onBlurCapture={(event) => {
                if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setSidebarOpen(false);
            }}
        >
            {/* Scrollable sidebar content — fills all space above the bottom bar */}
            <div className="flex-1 min-h-0 overflow-hidden">
                <AnimatePresence mode="wait">
                    {activeTab === 'chat' ? (
                        <ChatSidebar key="chat-sidebar" />
                    ) : isThinClawMode ? (
                        <ThinClawSidebarSlice key="thinclaw-sidebar" />
                    ) : isImagineMode ? (
                        <ImagineSidebarSlice key="imagine-sidebar" />
                    ) : isSettingsMode ? (
                        <SettingsSidebarSlice key="settings-sidebar" />
                    ) : null}
                </AnimatePresence>
            </div>

            {/* Mode Navigator — always pinned to the bottom */}
            <div className="shrink-0">
                <ModeNavigator
                    activeMode={appMode}
                    onModeChange={(mode) => {
                        if (mode === 'settings') {
                            setActiveTab('models');
                        } else {
                            setActiveTab(mode);
                        }
                    }}
                    onOpenPalette={openCommandPalette}
                    sidebarOpen={sidebarOpen}
                    gatewayRunning={thinclawGatewayRunning}
                />
            </div>
        </aside>
    );
}
