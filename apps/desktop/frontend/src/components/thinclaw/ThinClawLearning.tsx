...
export function ThinClawLearning() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [
                ...
                thinclaw.getLearningStatus(50),
                thinclaw.getLearningHistory(50),
                thinclaw.getLearningCandidates(50),
                thinclaw.getLearningArtifactVersions(50),
                thinclaw.getLearningProviderHealth(),
                thinclaw.getLearningCodeProposals(proposalFilter, 50),
                thinclaw.getLearningOutcomes(outcomeFilter, 50),
                thinclaw.getLearningRollbacks(50),
            ]);
    ...
    const submitRollback = async () => {
        ...
        await runAction('Recording rollback', () => thinclaw.recordLearningRollback(
                                ...
                                return (
                                    <div key={id || index} className="rounded-lg border border-border/40 p-3">
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-2">
                                            <button onClick={() => id && runAction('Approving proposal', () => thinclaw.reviewLearningCodeProposal(id, 'approve'))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/20">
                                                <Check className="w-3 h-3" />
                                                Approve
                                            </button>
                                            <button onClick={() => id && runAction('Rejecting proposal', () => thinclaw.reviewLearningCodeProposal(id, 'reject'))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/20">
                                                <X className="w-3 h-3" />
                                                Reject
                                            </button>
                                        </div>
                            ...
                            {outcomes.map((outcome, index) => {
                                ...
                                return (
                                    <div key={id || index} className="rounded-lg border border-border/40 p-3">
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-2">
                                            <button onClick={() => id && runAction('Confirming outcome', () => thinclaw.reviewLearningOutcome(id, 'confirm', 'positive'))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/20">
                                                <ThumbsUp className="w-3 h-3" />
                                                Positive
                                            </button>
                                            <button onClick={() => id && runAction('Confirming outcome', () => thinclaw.reviewLearningOutcome(id, 'confirm', 'negative'))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/20">
