 ...
 *
 * Listens for CanvasAction events (via `thinclaw-event` with `content_type: "canvas_action"`)
...
import { dismissCanvasPanel, getCanvasPanel, listCanvasPanels } from '../../../lib/thinclaw';
...
export function CanvasProviderWrapper({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        const setup = async () => {
            ...
            const u1 = await listen('thinclaw-canvas-push', (event: any) => {
            ...
            const u2 = await listen('thinclaw-canvas-navigate', (event: any) => {
