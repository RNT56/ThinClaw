...
function createEmptyRule(priority: number): thinclaw.RoutingRule {
    return {
        id: generateId(),
        label: '',
        match_kind: 'keyword',
...
function formatPoolDraft(pool: string[] | undefined): string {
    return (pool ?? []).join('\n');
}

function parsePoolDraft(value: string): string[] {
...
interface RuleCardProps {
    rule: thinclaw.RoutingRule;
    index: number;
    onUpdate: (id: string, patch: Partial<thinclaw.RoutingRule>) => void;
...
export function ThinClawRouting() {
    const [smartRoutingEnabled, setSmartRoutingEnabled] = useState(false);
    const [rules, setRules] = useState<thinclaw.RoutingRule[]>([]);
    ...
    const [simulationResult, setSimulationResult] = useState<thinclaw.RouteSimulationResponse | null>(null);
    const [simulationLoading, setSimulationLoading] = useState(false);
    const [primaryPoolDraft, setPrimaryPoolDraft] = useState('');
    const [cheapPoolDraft, setCheapPoolDraft] = useState('');
    const [poolSaving, setPoolSaving] = useState(false);
    const originalRef = useRef<string>('');

    const loadRoutingStatus = useCallback(async () => {
        try {
            const status = await thinclaw.getRoutingStatus();
            setRoutingStatus(status);
        } catch (e) {
            console.warn('[ThinClawRouting] Failed to load routing status:', e);
        }
    }, []);
    ...
    useEffect(() => {
        setPrimaryPoolDraft(formatPoolDraft(routingStatus?.primary_pool_order));
        setCheapPoolDraft(formatPoolDraft(routingStatus?.cheap_pool_order));
    }, [routingStatus?.primary_pool_order, routingStatus?.cheap_pool_order]);
    ...
    const handleToggle = async () => {
        ...
        try {
            await thinclaw.setRoutingConfig(next);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await thinclaw.saveRoutingRules(reindexed);
            setRules(reindexed);
    ...
    const handleUpdateRule = useCallback((id: string, patch: Partial<thinclaw.RoutingRule>) => {
    ...
    const handleReorder = (newOrder: thinclaw.RoutingRule[]) => {
        setRules(newOrder);
    };
    ...
    const handleSimulateRoute = async () => {
        ...
        try {
            const result = await thinclaw.simulateRouting({
    ...
    const handleSavePools = async () => {
        setPoolSaving(true);
        try {
            await thinclaw.saveRoutingPools(
                parsePoolDraft(primaryPoolDraft),
                parsePoolDraft(cheapPoolDraft),
            );
            await loadRoutingStatus();
            toast.success('Saved provider pools');
        } catch (e) {
            toast.error(`Failed to save provider pools: ${e}`);
        } finally {
            setPoolSaving(false);
        }
    };
    ...
    const poolsDirty = primaryPoolDraft !== formatPoolDraft(routingStatus?.primary_pool_order)
        || cheapPoolDraft !== formatPoolDraft(routingStatus?.cheap_pool_order);
                    ...
                    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3 space-y-3">
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <div className="mt-1 text-xs text-muted-foreground">One provider slug per line, evaluated top to bottom.</div>
                            </div>
                            <button
                                onClick={handleSavePools}
                                disabled={poolSaving || !poolsDirty}
                            ...
                            >
                                {poolSaving ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                                Save Pools
                            </button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <label className="space-y-1.5">
                                <span className="text-[11px] font-medium uppercase tracking-widest text-muted-foreground/60">Primary Pool</span>
                                <textarea
                                    value={primaryPoolDraft}
                            ...
                            <label className="space-y-1.5">
                                <span className="text-[11px] font-medium uppercase tracking-widest text-muted-foreground/60">Cheap Pool</span>
                                <textarea
                                    value={cheapPoolDraft}
