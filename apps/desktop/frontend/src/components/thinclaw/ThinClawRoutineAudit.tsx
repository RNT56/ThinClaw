...
export function ThinClawRoutineAudit({ routineKey }: Props) {
    const [entries, setEntries] = useState<thinclaw.RoutineAuditEntry[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [filter, setFilter] = useState<'all' | 'success' | 'failure'>('all');
    const [selectedKey, setSelectedKey] = useState(routineKey || '');
    const [cronJobs, setCronJobs] = useState<thinclaw.CronJob[]>([]);
    ...
    const fetchAudit = useCallback(async () => {
        ...
        try {
            ...
            const data = await thinclaw.getRoutineAuditList(selectedKey, 50, outcome as any);
            setEntries(data);
        } catch (e) {
    ...
    const handleRunSelected = async () => {
        if (!selectedKey) return;
        try {
            await thinclaw.runThinClawCron(selectedKey);
    ...
    const handleToggleSelected = async () => {
        ...
        try {
            await thinclaw.toggleRoutine(selectedJob.key, enabled);
                    ...
                    {confirmClear ? (
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-red-500/10 border border-red-500/20">
                            ...
                            <button
                                onClick={async () => {
                                    try {
                                        await thinclaw.clearRoutineRuns(selectedKey || undefined);
                                        setEntries([]);
                                        fetchAudit();
                                    } catch (e) {
