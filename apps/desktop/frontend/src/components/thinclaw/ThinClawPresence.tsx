...
interface SubAgentRowProps {
    info: thinclaw.ChildSessionInfo;
}
...
export function ThinClawPresence() {
    const [presence, setPresence] = useState<thinclaw.AgentRuntimePresence | null>(null);
    const [sessions, setSessions] = useState<thinclaw.ThinClawSession[]>([]);
    const [subAgents, setSubAgents] = useState<thinclaw.ChildSessionInfo[]>([]);
    ...
    const fetchAll = useCallback(async () => {
        try {
            const pres = await thinclaw.getThinClawSystemPresence();
            setPresence(pres);

            const sess = await thinclaw.getThinClawSessions();
            ...
            try {
                const children = await thinclaw.listChildSessions('agent:main');
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1 text-sm">
                        Live introspection of the embedded ThinClaw engine.
                        {lastFetched && (
