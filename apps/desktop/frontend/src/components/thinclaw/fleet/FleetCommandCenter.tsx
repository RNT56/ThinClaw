...
export function FleetCommandCenter() {
    ...
    const getProgress = useCallback((agent: thinclaw.AgentStatusSummary): number => {
        const rtState = agentStates[agent.id];
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        let yOffset = 50;
        const placeNode = (agent: thinclaw.AgentStatusSummary, depth: number, x: number) => {
    ...
    const handleSpawnTask = async (e: React.FormEvent) => {
        ...
        if (task && selectedAgentId) {
            setIsSpawning(true);
            try {
                await thinclaw.spawnSession(selectedAgentId, task);
                input.value = '';
                                                            ...
                                                            onClick={async () => {
                                                                if (agent.active_session_id) {
                                                                    try {
                                                                        ...
                                                                        await thinclaw.abortSession(agent.active_session_id, runId);
