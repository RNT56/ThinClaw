...
export function ThinClawChatView({ sessionKey, gatewayRunning, bootstrapNeeded = false, onBootstrapComplete = () => { }, onFactoryReset, onNavigateToSettings, onViewSession }: ThinClawChatViewProps) {
    const [messages, setMessages] = useState<ThinClawMessage[]>([]);
    ...
    const { status: runtimeStatus } = useThinClawStatusSnapshot(20000);
    ...
    const fetchHistory = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const res = await thinclaw.getThinClawHistory(effectiveSessionKey, 50);
            setMessages(res.messages);
            setTimeout(() => scrollToBottom('auto'), 100);
        } catch (e) {
            console.error('[ThinClawChatView] Failed to fetch history:', e);
        } finally {
            setIsLoading(false);
        }
    ...
    const updateMessagesFromEvent = (uiEvent: any) => {
        setMessages((prev: ThinClawMessage[]) => {
            if (uiEvent.kind === 'AssistantInternal') {
                ...
                return [...prev, { id: uiEvent.message_id, role: 'system', ts_ms: Date.now(), text: content, source: 'thinclaw', metadata: { type: 'internal' } }];
            }
            if (uiEvent.kind === 'AssistantSnapshot' || uiEvent.kind === 'AssistantFinal') {
                ...
                return [...prev, { id: uiEvent.message_id, role: 'assistant', ts_ms: Date.now(), text: uiEvent.text, source: 'thinclaw' }];
            }
            if (uiEvent.kind === 'AgentMessage') {
                ...
                return [...prev, { id: uiEvent.message_id, role: 'assistant', ts_ms: Date.now(), text: uiEvent.content, source: 'thinclaw', metadata }];
            }
            if (uiEvent.kind === 'AssistantDelta') {
                const existing = prev.find(m => m.id === uiEvent.message_id);
                ...
                return [...prev, { id: uiEvent.message_id, role: 'assistant', ts_ms: Date.now(), text: uiEvent.delta, source: 'thinclaw' }];
            }
            if (uiEvent.kind === 'ToolUpdate') {
                ...
                return [...prev, { id: toolMsgId, role: 'system', ts_ms: Date.now(), text: content, source: 'thinclaw', metadata }];
            }
            if (uiEvent.kind === 'PlanUpdate') {
                ...
                return [...prev, { id: planMsgId, role: 'system', ts_ms: Date.now(), text: content, source: 'thinclaw', metadata }];
            }
            if (uiEvent.kind === 'UsageUpdate') {
                ...
                return [...prev, { id: usageMsgId, role: 'system', ts_ms: Date.now(), text: content, source: 'thinclaw', metadata }];
            }
            ...
            if (uiEvent.kind === 'JobUpdate') {
                ...
                return [...prev, { id: jobMsgId, role: 'system', ts_ms: Date.now(), text: content, source: 'thinclaw', metadata }];
            }
            return prev;
        });
    }
    ...
    const handleToggleThinking = async () => {
        ...
        try {
            await thinclaw.setThinking(next, next ? thinkingBudget : undefined);
    ...
    const handleExportSession = async (format?: 'md' | 'json' | 'txt' | 'csv' | 'html') => {
        ...
        try {
            const result = await thinclaw.exportSession(effectiveSessionKey, fmt);
            if (fmt === 'md' || fmt === 'txt') {
    ...
    const handleSend = async () => {
        ...
        setIsSending(true);
        // Optimistic update
        const optimisticMsg: ThinClawMessage = { id: `temp-${Date.now()}`, role: 'user', ts_ms: Date.now(), text: msg, source: 'thinclaw' };
    ...
    const handleAbort = async () => {
        if (!effectiveSessionKey) return;
        try {
            await thinclaw.abortThinClawChat(effectiveSessionKey, currentRunId || undefined);
    ...
    const handleDeleteSession = async () => {
        ...
        try {
            ...
            if (onNavigateToSettings) onNavigateToSettings('thinclaw-gateway');
        } catch (e: any) {
            ...
            <div className={cn(
                ...
                <div className="absolute top-0 left-0 right-0 z-30 border-b border-border/50 px-6 py-3 flex items-center justify-between bg-background/60 backdrop-blur-xl">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {!gatewayRunning && (
                            <button onClick={() => onNavigateToSettings?.('thinclaw-gateway')} className="flex items-center gap-1.5 px-2 py-1 bg-amber-500/10 text-amber-500 text-[10px] font-bold border border-amber-500/20">
                                                    ...
                                                    >
                                                        ...
                                                        <input
                                                            ...
                                                            onMouseUp={async () => {
                                                                if (thinkingEnabled) {
                                                                    try {
                                                                        await thinclaw.setThinking(true, thinkingBudget);
