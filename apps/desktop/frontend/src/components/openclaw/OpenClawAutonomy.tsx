...
function CheckRow({ check }: { check: openclaw.AutonomyCheckResult }) {
    return (
...
export function OpenClawAutonomy() {
    const [status, setStatus] = useState<openclaw.AutonomyStatus | null>(null);
    const [permissions, setPermissions] = useState<unknown>(null);
    const [rollouts, setRollouts] = useState<openclaw.AutonomyRolloutSummary | null>(null);
    const [checks, setChecks] = useState<openclaw.AutonomyChecksSummary | null>(null);
    const [evidence, setEvidence] = useState<openclaw.AutonomyEvidenceSummary | null>(null);
    ...
    const load = useCallback(async () => {
        ...
        const [
            ...
            openclaw.getAutonomyStatus().catch((err) => {
                errors.push(errorText(err));
                return null;
            }),
            openclaw.getAutonomyPermissions().catch((err) => ({ unavailable: errorText(err) })),
            openclaw.getAutonomyRollouts().catch((err) => {
                errors.push(errorText(err));
                return null;
            }),
            openclaw.getAutonomyChecks().catch((err) => {
                errors.push(errorText(err));
                return null;
            }),
            openclaw.getAutonomyEvidence().catch((err) => {
                errors.push(errorText(err));
                return null;
            }),
        ]);
            ...
            <div className="rounded-lg border border-border/40 bg-card/30 p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button
                            disabled={isMutating}
                            onClick={() => runMutation('Bootstrap', openclaw.bootstrapAutonomy)}
                        ...
                        <button
                            ...
                            onClick={() => runMutation('Pause', () => openclaw.pauseAutonomy('Paused from ThinClaw Desktop'))}
                        ...
                        <button
                            disabled={!canMutate || isMutating || !status?.paused}
                            onClick={() => runMutation('Resume', openclaw.resumeAutonomy)}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-white/[0.03] border border-white/5 hover:bg-white/5 disabled:opacity-40"
                        >
                        ...
                        <button
                            ...
                            onClick={() => runMutation('Rollback', openclaw.rollbackAutonomy)}
