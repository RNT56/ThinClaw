...
export function OpenClawExperiments() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [
                ...
                openclaw.getExperimentProjects(),
                openclaw.getExperimentCampaigns(),
                openclaw.getExperimentRunners(),
                openclaw.getExperimentTargets(),
                openclaw.getExperimentModelUsage(100),
                openclaw.getExperimentOpportunities(100),
                openclaw.getExperimentGpuClouds(),
            ]);
    ...
    useEffect(() => {
        ...
        openclaw.getExperimentTrials(selectedCampaign)
            .then((trials) => {
    ...
    useEffect(() => {
        ...
        openclaw.getExperimentTrialArtifacts(selectedTrial)
                                            ...
                                            {(['pause', 'resume', 'cancel', 'promote', 'reissue-lease'] as const).map(action => (
                                                <span
                                                    ...
                                                    onClick={(event) => {
                                                        ...
                                                        if (id) runAction(`${action} campaign`, () => openclaw.runExperimentCampaignAction(id, action));
                                                    }}
                                ...
                                return (
                                    <div key={id || index} className="rounded-lg border border-border/40 p-3">
                                        <div className="flex items-center justify-between gap-3">
                                            ...
                                            <button
                                                onClick={() => id && runAction('Validating runner', () => openclaw.validateExperimentRunner(id))}
                                ...
                                return (
                                    <div key={slug || index} className="rounded-lg border border-border/40 p-3">
                                        ...
                                        <div className="mt-3 flex gap-2">
                                            <button onClick={() => slug && runAction('Validating GPU cloud', () => openclaw.validateExperimentGpuCloud(slug))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-white/[0.03] hover:bg-white/[0.06] border border-white/5">
                                                <ShieldCheck className="w-3 h-3" />
                                                Validate
                                            </button>
                                            <button onClick={() => slug && runAction('Launching test', () => openclaw.launchExperimentGpuCloudTest(slug))} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-medium bg-white/[0.03] hover:bg-white/[0.06] border border-white/5">
