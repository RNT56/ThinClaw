...
pub async fn direct_media_transcribe_audio(
    ...
    if let Some((port, token, model_family)) = server_config {
        ...
        let asset =
            persist_voice_input(&app, pool.inner(), &audio_bytes, Some(model_family), &text)
                .await?;
        return Ok(DirectSttResponse { text, asset });
    }
    ...
    #[cfg(not(feature = "mlx"))]
    {
        ...
        if !output.status.success() {
            ...
            return Err((format!("Whisper Error: {}", stderr)).into());
        }
        ...
        if std::path::Path::new(&output_path).exists() {
            ...
            Err(("No output file generated by whisper".to_string()).into())
        }
    }
}
...
async fn persist_voice_input(
    ...
    let relative_path = format!("voice/input/{}.wav", id);
    file_store
