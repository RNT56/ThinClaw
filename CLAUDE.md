...
- `thinclaw-tools` owns tool registry core, MCP protocol/config/session primitives, WASM tool primitives, the WASM rate limiter, and WASM credential injection. Root `src/tools` still owns the host execution pipeline, root-dependent built-ins, WASM wrapper/loader/oauth/storage, and app-specific registration.
