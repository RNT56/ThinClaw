//! WebSocket client for OpenClawEngine Gateway
//!
//! Implements:
...
impl OpenClawWsHandle {
    ...
    pub async fn chat_abort(
        ...
        self.rpc("chat.abort", params).await
    }
...
impl OpenClawWsClient {
    ...
    pub async fn run_forever(mut self) {
            ...
            match self.run_once().await {
                ...
                Err(e) => {
                    ...
                    if self.connection_attempts <= 5 {
                        info!("WS connection attempt {}: {} (openclaw-engine may still be starting...)", 
                              self.connection_attempts, e);
                    } else {
                        error!(
                            "WS connection error (attempt {}): {}",
                            self.connection_attempts, e
                        );
                    }
        ...
        info!("OpenClaw WS client shutting down");
    }

    /// Run a single connection attempt
    async fn run_once(&mut self) -> Result<(), ClientError> {
        info!("Connecting to OpenClawEngine gateway: {}", self.gateway_url);
        ...
        if let (Some(nonce), Some(pem)) = (nonce.as_ref(), self.private_key_pem.as_ref()) {
            if let Ok(signing_key) = SigningKey::from_pkcs8_pem(pem) {
                // MATCH OPENCLAW buildDeviceAuthPayload
