//! Root-independent outbound media helper policy.
...
use thinclaw_media::MediaLimits;
use thinclaw_tools_core::ToolArtifact;
use thinclaw_types::{MediaContent, MediaType};
...
pub async fn attachments_from_tool_result(
    tool_name: &str,
    result_json: &serde_json::Value,
    artifacts: &[ToolArtifact],
) -> Vec<MediaContent> {
    if !is_generated_media_tool(tool_name) {
        return Vec::new();
    }
...
pub fn is_generated_media_tool(tool_name: &str) -> bool {
    GENERATED_MEDIA_TOOLS.contains(&tool_name)
}

pub fn dedupe_extend(target: &mut Vec<MediaContent>, incoming: Vec<MediaContent>) {
    let mut seen = target
        .iter()
...
pub fn dedupe_attachments(attachments: Vec<MediaContent>) -> Vec<MediaContent> {
...
pub fn supported_generated_media_mime(mime_type: &str) -> bool {
    MediaType::from_mime(mime_type) != MediaType::Unknown
}
...
mod tests {
    ...
    #[test]
    fn generated_media_tool_allowlist_is_exact() {
        assert!(is_generated_media_tool("image_generate"));
        assert!(is_generated_media_tool("comfy_run_workflow"));
        assert!(!is_generated_media_tool("shell"));
    }

    #[test]
    fn dedupe_extend_preserves_first_attachment_order() {
        let first = MediaContent::new(b"same".to_vec(), "image/png").with_filename("first.png");
        let duplicate =
            MediaContent::new(b"same".to_vec(), "image/png").with_filename("duplicate.png");
        let second = MediaContent::new(b"other".to_vec(), "image/png").with_filename("other.png");

        let mut target = vec![first];
        dedupe_extend(&mut target, vec![duplicate, second]);

        assert_eq!(target.len(), 2);
        assert_eq!(target[0].filename.as_deref(), Some("first.png"));
        assert_eq!(target[1].filename.as_deref(), Some("other.png"));
    }

    #[test]
    fn basename_and_mime_policy_match_runtime_expectations() {
        assert_eq!(
            safe_basename("../rendered.png").as_deref(),
            Some("rendered.png")
        );
        assert_eq!(safe_basename(".."), None);
        assert!(supported_generated_media_mime("image/png"));
        assert!(!supported_generated_media_mime("application/octet-stream"));
    ...
    async fn generated_outputs_reject_unapproved_paths_and_use_artifact_fallback() {
        ...
        let artifact = ToolArtifact::Image {
            data: base64::engine::general_purpose::STANDARD.encode(b"fallback"),
        ...
        assert_eq!(attachments.len(), 1);
        assert_eq!(attachments[0].data, b"fallback");
