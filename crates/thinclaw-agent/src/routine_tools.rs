...
use crate::ports::RoutineStorePort;
use crate::routine::{
...
#[async_trait]
pub trait RoutineEngineControlPort: Send + Sync {
    async fn refresh_event_cache(&self);
}

#[async_trait]
pub trait RoutineOutcomeObserver: Send + Sync {
...
pub struct RoutineCreateTool {
    store: Arc<dyn RoutineStorePort>,
    engine: Arc<dyn RoutineEngineControlPort>,
}
...
impl RoutineListTool {
    pub fn new(store: Arc<dyn RoutineStorePort>) -> Self {
        Self { store }
    }
}
...
pub struct RoutineUpdateTool {
    store: Arc<dyn RoutineStorePort>,
    engine: Arc<dyn RoutineEngineControlPort>,
    outcome_observer: Option<Arc<dyn RoutineOutcomeObserver>>,
}
...
impl RoutineUpdateTool {
    ...
    pub fn with_outcome_observer(mut self, observer: Arc<dyn RoutineOutcomeObserver>) -> Self {
...
impl Tool for RoutineUpdateTool {
    ...
    async fn execute(
        ...
        if !routine.enabled
            && let Some(observer) = self.outcome_observer.as_ref()
        {
            observer
                .observe_state_change(&routine, "routine_disabled")
                .await;
        }
...
impl Tool for RoutineDeleteTool {
    ...
    async fn execute(
        ...
        if deleted && let Some(observer) = self.outcome_observer.as_ref() {
            observer
                .observe_state_change(&routine, "routine_deleted")
                .await;
        }
