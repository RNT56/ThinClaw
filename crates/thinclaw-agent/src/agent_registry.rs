...
use super::agent_router::{AgentRouter, AgentWorkspace};
...
#[async_trait]
pub trait AgentRegistryStorePort: Send + Sync {
...
#[async_trait]
pub trait AgentWorkspaceSeeder: Send + Sync {
...
pub struct AgentRegistry {
    ...
    store: Option<Arc<dyn AgentRegistryStorePort>>,
    seeder: Option<Arc<dyn AgentWorkspaceSeeder>>,
}

impl AgentRegistry {
    /// Create a new registry. Pass None for db to operate in-memory only.
    pub fn new(router: Arc<AgentRouter>, store: Option<Arc<dyn AgentRegistryStorePort>>) -> Self {
        Self {
            router,
            store,
            seeder: None,
    ...
    pub fn with_seeder(mut self, seeder: Option<Arc<dyn AgentWorkspaceSeeder>>) -> Self {
        self.seeder = seeder;
        self
    }
    ...
    pub async fn load_from_db(&self) -> Result<usize, AgentRegistryError> {
        ...
        let records = store.list_agent_workspaces().await?;
    ...
    pub async fn create_agent(
        ...
        if let Some(ref store) = self.store {
            store.save_agent_workspace(&record).await?;
        }
    ...
    pub async fn remove_agent(
        ...
        // Remove from DB
        if let Some(ref store) = self.store {
            store.delete_agent_workspace(agent_id).await?;
        }
    ...
    pub async fn update_agent(
        ...
        let mut record = if let Some(ref store) = self.store {
            store
        ...
        // Persist
        if let Some(ref store) = self.store {
            store.update_agent_workspace(&record).await?;
        }
    ...
    pub async fn get_agent_record(
        ...
        if let Some(ref store) = self.store {
            Ok(store.get_agent_workspace(agent_id).await?)
        } else {
            Ok(self
                .router
                .get_agent(agent_id)
    ...
    async fn seed_workspace(&self, record: &AgentWorkspaceRecord) {
        ...
        if let Err(e) = seeder.seed_workspace(record).await {
            tracing::warn!(
                agent_id = %record.agent_id,
                error = %e,
