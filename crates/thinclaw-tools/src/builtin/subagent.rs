...
use thinclaw_types::{
    JobContext, SubagentMemoryMode, SubagentSkillMode, SubagentTaskPacket, SubagentToolMode,
    ToolProfile,
};
...
#[async_trait]
pub trait SubagentToolPort: Send + Sync {
...
impl Tool for ListSubagentsTool {
    ...
    async fn execute(
        ...
        let start = Instant::now();
        let agents = self.executor.list_subagents().await;
        ...
        let running = agents
            .iter()
            .filter(|agent| agent.get("status").and_then(|value| value.as_str()) == Some("Running"))
            .count();
...
impl Tool for CancelSubagentTool {
    ...
    async fn execute(
        ...
        let cancelled = self.executor.cancel_subagent(agent_id).await;
