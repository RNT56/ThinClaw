...
use thinclaw_tools_core::tool::{
    Tool, ToolError, ToolMetadata, ToolOutput, ToolRouteIntent, require_str,
};
...
fn resolve_requested_timezone(
    ...
    match params.get("timezone").and_then(|value| value.as_str()) {
        ...
        Some(raw) => {
            let tz = thinclaw_platform::timezone::parse_timezone(raw).ok_or_else(|| {
                ToolError::InvalidParameters(format!(
                    "invalid timezone '{}'; use an IANA timezone like 'Europe/Berlin' or a fixed offset like 'GMT+1'",
                    raw
                ))
            })?;
            let label = thinclaw_platform::timezone::normalize_timezone_label(raw)
