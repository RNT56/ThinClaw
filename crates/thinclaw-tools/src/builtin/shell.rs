...
#[cfg(test)]
use crate::execution::MAX_CAPTURED_OUTPUT_SIZE;
use crate::execution::{
    CommandExecutionRequest, ExecutionBackendKind, ExecutionResult, LocalExecutionBackend,
    LocalHostExecutionBackend, RuntimeDescriptor,
};
use crate::smart_approve::{ApprovalDecision, SmartApprovalMode};
...
pub use super::shell_security::{
    ExternalScannerMode, detect_command_injection, detect_library_injection,
...
pub struct ShellSafetyOptions {
    pub smart_approval_mode: Option<SmartApprovalMode>,
...
#[async_trait]
pub trait ShellSmartApprover: Send + Sync {
...
#[async_trait]
pub trait AcpTerminalExecutor: Send + Sync {
...
pub struct ShellTool {
    ...
    sandbox_backend: Option<Arc<dyn LocalExecutionBackend>>,
    /// Shared local execution backend.
    local_backend: Arc<dyn LocalExecutionBackend>,
    ...
    smart_approver: Option<Arc<dyn ShellSmartApprover>>,
...
impl std::fmt::Debug for ShellTool {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ShellTool")
            ...
            .field("sandbox_backend", &self.sandbox_backend.is_some())
...
impl ShellTool {
    ...
    /// Enable sandbox execution with a host-provided execution backend.
    pub fn with_sandbox_backend(mut self, backend: Arc<dyn LocalExecutionBackend>) -> Self {
        self.sandbox_backend = Some(backend);
        self
    }

    pub fn with_local_backend(mut self, backend: Arc<dyn LocalExecutionBackend>) -> Self {
        self.local_backend = backend;
        self
    }

    pub fn with_smart_approver(mut self, approver: Arc<dyn ShellSmartApprover>) -> Self {
        self.smart_approver = Some(approver);
        self
    }

    pub fn with_acp_terminal(mut self, terminal: Arc<dyn AcpTerminalExecutor>) -> Self {
        self.acp_terminal = Some(terminal);
        self
    }

    pub fn with_safety_options(mut self, options: &ShellSafetyOptions) -> Self {
        ...
        if scanner_mode != ExternalScannerMode::Off || options.external_scanner_path.is_some() {
            self.external_scanner = Some(ExternalCommandScanner::new_with_provenance_requirement(
                scanner_mode,
                options.external_scanner_path.clone(),
                require_verified,
            ));
        } else {
    ...
    pub fn with_external_scanner_require_verified(mut self, require_verified: bool) -> Self {
        if let Some(scanner) = self.external_scanner.as_ref() {
            self.external_scanner = Some(ExternalCommandScanner::new_with_provenance_requirement(
                scanner.mode(),
                scanner.configured_path().cloned(),
                require_verified,
            ));
        }
        self
    ...
    fn smart_approval_mode(&self) -> SmartApprovalMode {
        ...
        optional_env("SAFETY_SMART_APPROVAL_MODE")
            .or_else(|| optional_env("THINCLAW_SMART_APPROVAL_MODE"))
    ...
    async fn assess_soft_flag_command(
        ...
        let Some(approver) = self.smart_approver.as_ref() else {
            return ApprovalDecision::Escalate;
        };
    ...
    fn active_backend(&self) -> Arc<dyn LocalExecutionBackend> {
        if let Some(ref sandbox_backend) = self.sandbox_backend {
            return Arc::clone(sandbox_backend);
        }
    ...
    async fn execute_command(
        ...
        if let (Some(session_id), Some(acp_terminal)) = (
            ctx.metadata
                .get("acp_session_id")
                .and_then(serde_json::Value::as_str),
            self.acp_terminal.as_ref(),
        ) {
                ...
                .await
            {
                Ok(Some(execution)) => {
                    ...
                    return Ok(ExecutionResult {
                        ...
                        runtime: RuntimeDescriptor::logical_surface(
                            "acp_client_terminal",
                            "client_terminal",
                            "acp",
...
async fn evaluate_soft_flag_command(
    approver: Arc<dyn ShellSmartApprover>,
    command: String,
    reason: String,
    working_dir: String,
) -> ApprovalDecision {
            ...
            && let Some(reason) = self.soft_flag_reason(cmd)
        {
            ...
            match self.smart_approval_mode() {
                ...
                SmartApprovalMode::Smart => {
                    ...
                    let Some(approver) = self.smart_approver.clone() else {
                        return ApprovalRequirement::Always;
                    };
...
mod tests {
    ...
    #[async_trait]
    impl ShellSmartApprover for StaticApprover {
        async fn assess_command(
            &self,
            _command: &str,
            _description: &str,
    ...
    #[tokio::test]
    async fn test_external_scanner_require_verified_escalates_unverified_configured_scanner() {
        let dir = tempfile::tempdir().unwrap();
        let scanner_path = dir.path().join(if cfg!(windows) {
            "scanner.cmd"
        } else {
        ...
        std::fs::write(&scanner_path, "unused scanner").unwrap();

        let tool = ShellTool::new().with_safety_options(&ShellSafetyOptions {
        ...
        assert!(
            matches!(result, Err(ToolError::NotAuthorized(ref msg)) if msg.contains("provenance")),
            "Expected strict provenance to escalate unverified scanner, got: {result:?}"
        );
    }

    #[tokio::test]
    async fn test_external_scanner_require_verified_fail_open_allows_unverified_unknown() {
        let dir = tempfile::tempdir().unwrap();
        let scanner_path = dir.path().join(if cfg!(windows) {
            "scanner.cmd"
        } else {
