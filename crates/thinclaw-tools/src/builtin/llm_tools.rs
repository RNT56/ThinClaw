...
use thinclaw_tools_core::{Tool, ToolError, ToolOutput, require_str};
...
pub fn model_override_scope_key_from_metadata(
...
pub fn is_runtime_supported_provider_slug(provider_slug: &str) -> bool {
    thinclaw_config::provider_catalog::endpoint_for(provider_slug).is_some()
        || matches!(
            provider_slug,
...
pub fn wrap_model_spec_override(
...
impl Tool for LlmSelectTool {
    ...
    async fn execute(
        ...
        let endpoint = thinclaw_config::provider_catalog::endpoint_for(provider_slug);
        if endpoint.is_none() && !is_runtime_supported_provider_slug(provider_slug) {
            let available: Vec<&str> = thinclaw_config::provider_catalog::all_provider_ids();
            return Err(ToolError::InvalidParameters(format!(
        ...
        if let Some(endpoint) = endpoint {
            ...
            let has_key = thinclaw_config::helpers::optional_env(env_key)
                .ok()
                .flatten()
                .is_some();
            ...
            thinclaw_llm::provider_factory::probe_provider_model(provider_slug, model_name)
                .await
                .map_err(|err| {
...
impl Tool for LlmListModelsTool {
    ...
    async fn execute(
        ...
        let mut entries: Vec<(String, String, String, u32, bool, bool)> =
            thinclaw_config::provider_catalog::catalog()
                .iter()
                .map(|(slug, endpoint)| {
                    let has_key = thinclaw_config::helpers::optional_env(&endpoint.env_key_name)
                        .ok()
                        .flatten()
                        .is_some();
                    (
        ...
        entries.extend([
            ...
            (
                ...
                thinclaw_config::helpers::optional_env("BEDROCK_API_KEY")
                    ...
                    .flatten()
                    .is_some()
                    || thinclaw_config::helpers::optional_env("AWS_BEARER_TOKEN_BEDROCK")
                        .ok()
                        .flatten()
                        .is_some()
                    || thinclaw_config::helpers::optional_env("BEDROCK_PROXY_URL")
                        .ok()
                        .flatten()
                        .is_some(),
            ),
