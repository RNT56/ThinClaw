...
use crate::execution::{
    ExecutionBackendKind, ExecutionResult, LocalExecutionBackend, LocalHostExecutionBackend,
    NetworkIsolationKind, RuntimeDescriptor, ScriptExecutionRequest, host_local_network_isolation,
};
use crate::registry::ToolRegistry;
use thinclaw_tools_core::{
    ApprovalRequirement, RateLimitResult, Tool, ToolDomain, ToolError, ToolOutput,
    ToolRateLimitConfig, require_str,
};
...
pub struct ExecuteCodeTool {
    ...
    tool_rpc_host: Option<Arc<dyn ToolRpcHost>>,
}

#[async_trait]
pub trait ToolRpcHost: Send + Sync {
...
struct RegistryToolRpcHost {
    tools: Weak<ToolRegistry>,
}

impl RegistryToolRpcHost {
...
#[async_trait]
impl ToolRpcHost for RegistryToolRpcHost {
    async fn execute_tool_rpc(
        &self,
        ctx: &JobContext,
        tool_name: &str,
        ...
        execute_inner_tool_rpc_from_registry(&self.tools, ctx, tool_name, params).await
    }
}
...
impl ExecuteCodeTool {
    ...
    pub fn with_tool_registry(mut self, tools: Weak<ToolRegistry>) -> Self {
        self.tool_rpc_host = Some(Arc::new(RegistryToolRpcHost::new(tools)));
        self
    }

    pub fn with_tool_rpc_host(mut self, host: Arc<dyn ToolRpcHost>) -> Self {
        self.tool_rpc_host = Some(host);
        self
    }
    ...
    async fn execute_tool_rpc_code(
        ...
        let tool_rpc_host = self.tool_rpc_host.clone().ok_or_else(|| {
        ...
        let server = tokio::spawn(serve_tool_rpc(
            rpc_dir.clone(),
            tool_rpc_host,
            rpc_ctx,
            shutdown_rx,
...
async fn handle_tool_rpc_request_file(
    ...
    let response = match request {
        Ok(request) => match execute_inner_tool_rpc(tool_rpc_host, ctx, request).await {
...
async fn handle_tool_rpc_stream_windows(
    ...
    let response = match execute_inner_tool_rpc(&tool_rpc_host, &ctx, request).await {
        Ok(result) => ToolRpcResponse {
            ok: true,
            result: Some(result),
            error: None,
...
async fn execute_inner_tool_rpc(
    ...
    let start = std::time::Instant::now();
    let result = tool_rpc_host
        .execute_tool_rpc(ctx, &request.name, request.params.clone())
        .await?;
...
async fn execute_inner_tool_rpc_from_registry(
    tools: &Weak<ToolRegistry>,
    ctx: &JobContext,
    tool_name: &str,
    params: serde_json::Value,
    ...
    let tools = tools.upgrade().ok_or_else(|| {
        ToolError::ExecutionFailed(
            "tool_rpc mode is unavailable because the tool registry is not wired".to_string(),
        )
    })?;
    ...
    if let Some(config) = tool.rate_limit_config()
        && let RateLimitResult::Limited { retry_after, .. } = tools
            .rate_limiter()
...
pub fn tool_rpc_auto_approves(tool_name: &str, params: &serde_json::Value) -> bool {
    match tool_name {
...
mod tests {
    ...
    async fn test_execute_inner_tool_rpc_blocks_disallowed_tools() {
        let registry = Arc::new(ToolRegistry::new());
        let ctx = JobContext::default();
        let host: Arc<dyn ToolRpcHost> =
            Arc::new(RegistryToolRpcHost::new(Arc::downgrade(&registry)));
