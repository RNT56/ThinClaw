...
pub async fn authorize_mcp_server_with_options(
    ...
    let host = options.callback_host.clone();
    ...
    if !is_loopback_host(&host) {
        (options.on_remote_plain_http_callback)(&host, port);
    }
    ...
    if let Err(e) = (options.open_authorization_url)(&auth_url) {
        println!("  Could not open browser: {}", e);
        (options.on_manual_authorization_url)(&auth_url);
    }
    ...
    if is_loopback_host(host) {
        ...
        match TcpListener::bind(&ipv4_addr).await {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
                return Err(AuthError::PortUnavailable);
            }
            Err(_) => {}
        }
