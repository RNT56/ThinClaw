...
pub struct CompletionResponse {
    ...
    /// Provider-native exact token/logprob data, when the upstream response
    /// includes it. This must never be filled with synthetic word splitting.
...
/// Exact token/logprob data returned by an upstream provider.
///
/// These fields intentionally mirror the trajectory artifact shape. Empty
/// vectors mean the provider did not expose that part of the data for this
/// response; callers should inspect the support flags before treating arrays as
/// complete.
#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq)]
pub struct ProviderTokenCapture {
    #[serde(default)]
    pub exact_tokens_supported: bool,
    #[serde(default)]
    pub logprobs_supported: bool,
...
pub enum StreamChunk {
    ...
    Done {
        ...
        /// Provider-native exact token/logprob data, if included in the final
...
pub trait LlmProvider: Send + Sync {
    ...
    async fn complete_stream(
        ...
        Ok(crate::streaming::simulate_stream_from_response(
            resp.content,
            resp.provider_model,
            resp.cost_usd,
...
mod tests {
    use super::*;
    use crate::streaming::simulate_stream_from_response;
    use async_trait::async_trait;
    use rust_decimal::Decimal;
