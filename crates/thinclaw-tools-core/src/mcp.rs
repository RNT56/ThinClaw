//! MCP wire-protocol DTOs (requests, responses, capabilities, tool/resource/prompt
//! descriptors, content blocks, notifications). Pure serde data — moved here from
//! `thinclaw-tools` so that light consumers (e.g. `thinclaw-gateway`) can use these
//! DTOs without depending on the heavyweight tool runtime (wasmtime/chromiumoxide).
//! `thinclaw_tools::mcp::protocol` re-exports this module for path stability.
