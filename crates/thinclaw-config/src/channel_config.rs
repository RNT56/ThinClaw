...
impl ChannelsConfig {
    ...
    #[cfg(feature = "nostr")]
    pub fn resolve_nostr(settings: &Settings) -> Result<Option<NostrConfig>, ConfigError> {
        ...
        let owner_pubkey = optional_env("NOSTR_OWNER_PUBKEY")?
            ...
            .map(|raw| {
                thinclaw_channels::nostr_runtime::normalize_public_key(&raw).map_err(|message| {
        ...
        let owner_pubkey = match owner_pubkey {
            ...
            {
                let migrated =
                    thinclaw_channels::nostr_runtime::normalize_public_key(&allow_from[0])
...
fn normalize_telegram_transport_mode(value: impl AsRef<str>) -> String {
    ...
    let resolution = crate::channels::resolve_telegram_transport_mode(value);
