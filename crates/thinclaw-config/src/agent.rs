...
pub fn resolve_personality_pack_from_settings(settings: &Settings) -> String {
...
pub fn canonical_personality_pack_name(requested: &str) -> &'static str {
    let canonical = normalize_personality_key(requested);
...
fn normalize_personality_key(value: &str) -> String {
    let normalized = value.trim().to_ascii_lowercase().replace(['-', ' '], "_");
    match normalized.as_str() {
        "default" | "natural" => "balanced".to_string(),
        "flow" => "flow_state".to_string(),
