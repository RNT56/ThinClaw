...
use thinclaw_types::error::{DatabaseError, LlmError};
...
async fn record_guard_only(
    guard: &Arc<dyn LlmBudgetRecorder>,
    fallback_model: &str,
...
async fn record_usage(
    ...
    guard: Option<&Arc<dyn LlmBudgetRecorder>>,
...
mod tests {
    ...
    use thinclaw_llm_core::{FinishReason, ToolCompletionRequest, ToolCompletionResponse};
    ...
    impl StubLlm {
        ...
        fn with_model_name(mut self, model: impl Into<String>) -> Self {
    ...
    struct TestBudgetRecorder {
        calls: AtomicU64,
    }

    impl TestBudgetRecorder {
        fn new() -> Self {
            Self {
    ...
    #[async_trait]
    impl LlmBudgetRecorder for TestBudgetRecorder {
        async fn record_llm_call_with_cost(
            &self,
            _model: &str,
            _input_tokens: u32,
    ...
    async fn direct_completion_is_recorded() {
        ...
        let guard = Arc::new(TestBudgetRecorder::new());
        let provider: Arc<dyn LlmProvider> = Arc::new(UsageTrackingProvider::new(
            ...
            Arc::clone(&tracker),
            None,
            Some(Arc::clone(&guard) as Arc<dyn LlmBudgetRecorder>),
        ));
