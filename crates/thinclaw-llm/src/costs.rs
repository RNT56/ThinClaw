...
/// Dynamic pricing overlay populated by the runtime pricing sync.
///
...
///
/// Called by runtime pricing sync after fetching from OpenRouter.
...
pub fn model_cost_with_source(model_id: &str) -> Option<(Decimal, Decimal, CostSource)> {
    ...
    if let Some(model) = thinclaw_config::model_compat::find_model(model_id)
