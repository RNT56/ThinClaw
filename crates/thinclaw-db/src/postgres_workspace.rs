    ...
    pub async fn append_document_by_path(
        ...
        let row = if agent_id.is_none() {
            conn.query_one(
                ...
                DO UPDATE SET
                    content = CASE WHEN memory_documents.content = '' THEN EXCLUDED.content
                                   ELSE memory_documents.content || $6 || EXCLUDED.content END,
                    updated_at = NOW(),
            ...
            conn.query_one(
                ...
                DO UPDATE SET
                    content = CASE WHEN memory_documents.content = '' THEN EXCLUDED.content
                                   ELSE memory_documents.content || $7 || EXCLUDED.content END,
                    updated_at = NOW(),
                    metadata = memory_documents.metadata || '{"index_dirty":true}'::jsonb
                RETURNING id, user_id, agent_id, path, content,
                          created_at, updated_at, metadata
