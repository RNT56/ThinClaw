...
> [!NOTE]
> **Sections 1-3** (Document ingestion, Memory, Web Search) remain accurate.
> **Section 4** (Chat Session Lifecycle) describes the **old Node.js/WebSocket bridge** which has been replaced by IronClaw (in-process Rust library). The WS handshake, normalizer pipeline, and `ws_client.rs` references in §4 are historical. For the current IronClaw data flow, see `documentation/latest/MICROSERVICES_AND_SIDECARS.md` §13.
