> [!WARNING]
> **📦 ARCHIVED — Pre-IronClaw Implementation Spec**
>
> This document describes the **old Node.js/WebSocket bridge** OpenClaw integration which has been **fully replaced**
> by IronClaw (in-process Rust library). This file is preserved as historical reference only.
>
> For current architecture, see:
> - `documentation/scrappy_documentation_canonical.md` (§12 — IronClaw)
> - `documentation/latest/MICROSERVICES_AND_SIDECARS.md`
> - `tauri_dropin_spec.md` (completed integration log)

# Implementation Progress (Updated: 2026-01-28)

### Phase 1: Foundation ✅ COMPLETE
- `Cargo.toml` → Added tokio-tungstenite, tungstenite, futures-util
- `src-tauri/src/openclaw/mod.rs` → Module structure
- `openclaw/frames.rs` → WsFrame models, request builders
- `openclaw/normalizer.rs` → UiEvent types, fallback parsing
- `openclaw/config.rs` → OpenClawEngineConfig, Slack/Telegram structs
- `openclaw/ws_client.rs` → Async client, reconnection, actor pattern

### Phase 2: UI Scaffolding ✅ COMPLETE
- `SettingsSidebar.tsx` → Added OpenClaw section (Slack, Telegram, Gateway)
...
- `SettingsPages.tsx` → Integrated all OpenClaw tabs

### Phase 3: Core Integration ✅ COMPLETE
- `openclaw/commands.rs` → OpenClawManager + 12 Tauri commands
- `lib.rs` → Registered commands in specta builder, OpenClawManager state
- `src/lib/openclaw.ts` → TypeScript API wrappers
- All 3 settings tabs wired to call Tauri commands

### Phase 4: Chat UI ✅ COMPLETE
- `ChatLayout.tsx` → 3-way mode switching (chat/openclaw/settings)
- `OpenClawSidebar.tsx` → Sessions list, gateway status, refresh
- `OpenClawChatView.tsx` → Message history, send, subscribe
- Radio icon + gateway status indicator in sidebar

---

Below is a comprehensive implementation spec for adding a standalone OpenClawEngine/OpenClaw mode inside your Scrappy Inference Tauri (Rust) desktop app, with a separate UI (clone of your existing chat UI) plus a settings tab for connectors/auth. It assumes macOS + Windows, Slack + Telegram first, and llama.cpp via local HTTP server.

I’m using “OpenClawEngine” for the upstream project name (OpenClaw → OpenClawEngine rebrand).  ￼

⸻

Scrappy Inference — Standalone “OpenClaw Mode” Spec (OpenClawEngine Integration)

1) Summary

Add a new feature area in Scrappy Inference:
	•	Mode: “OpenClaw” (standalone OpenClawEngine assistant)
	...
	•	Runtime: Scrappy supervises a bundled OpenClawEngine stack (Gateway + Agent loop) and points it at your local llama.cpp endpoint.

OpenClawEngine is designed as an always-on Gateway (WS control plane + channel owners) plus an agent loop that turns inbound messages into actions/replies.  ￼
...
2.1 Components (on the user’s machine)
	...
	•	Feature gating & UI
	•	Supervisor (starts/stops OpenClawEngine + monitors health)
	•	Native WS client to OpenClawEngine Gateway
	•	Settings & secrets management
	•	Log viewer
	2.	OpenClawEngine Gateway (bundled)
	...
	3.	OpenClawEngine Agent runtime (bundled, same package)
	•	Runs the OpenClawEngine agent loop (context assembly → inference → tool calls → persistence)  ￼
...
3.2 Local-only, authenticated gateway
	...
	•	Enforce gateway auth token; OpenClawEngine is fail-closed if not configured.  ￼
...
Add top-level section (or sidebar entry): OpenClaw with subpages:
	1.	Chat
	•	Session list (left)
	•	Transcript (center)
	•	Composer (bottom)
...
OpenClawEngine’s WebChat model is: chat.history + chat.send + best-effort push updates via chat.subscribe events.  ￼

Required behaviors
	•	On app open: connect WS, subscribe to chat events.
	•	Session list:
	•	show main session and group/channel sessions
	•	show labels per OpenClawEngine conventions (channel + token, etc.)  ￼
	•	Transcript:
...
From OpenClawEngine’s API categories:  ￼
	•	connect
...
Side-effecting methods require idempotency keys (OpenClawEngine enforces this for safety).  ￼
...
OpenClawEngine distinguishes:
	•	State dir (~/.openclaw by default) holding config, secrets, sessions, extensions  ￼
...
Set OpenClawEngine env vars so everything is contained inside Scrappy:

macOS
	•	Base: ~/Library/Application Support/Scrappy Inference/OpenClaw/
	•	state/
	•	workspace/
	•	logs/

Windows
	•	Base: %AppData%\Scrappy Inference\OpenClaw\
	•	state\
	•	workspace\
	•	logs\

6.2 Environment variables (set for OpenClawEngine processes)
	•	OPENCLAW_STATE_DIR=<.../state>
	•	OPENCLAW_CONFIG_PATH=<.../state/openclaw-engine.json>
	•	workspace set via config (agent.workspace) to <.../workspace>  ￼

6.3 Permissions and secrecy

OpenClawEngine docs explicitly warn that state dirs contain sensitive data (tokens, sessions, transcripts).  ￼
Spec:
...
Scrappy generates openclaw-engine.json and manages it (don’t ask the user to hand-edit). Base it on upstream schema (docs.molt.bot/gateway/configuration).  ￼
...
From OpenClawEngine Slack channel docs:
	•	Create Slack app
	•	Enable Socket Mode
...
Your UI should provide:
	•	a “Slack Manifest” copy button (pre-filled with the scopes/events OpenClawEngine expects)
	•	a checklist screen (so support tickets become trivial)

7.3 Telegram required Telegram-side setup (show in UI)

From OpenClawEngine Telegram docs:
...
Upstream OpenClawEngine is distributed as an npm package requiring Node ≥ 22.  ￼
So “shipping binaries” should mean Scrappy ships:
	1.	a Node 22 runtime (per-OS bundled)
	2.	the OpenClawEngine distribution (packaged JS bundle + node_modules, or a single compiled artifact)
	3.	launch scripts to run openclaw-engine gateway and openclaw-engine agent (or the unified entrypoint) as child processes

OpenClawEngine supports npm/pnpm/bun in dev, and “preferred setup is openclaw-engine onboard”.  ￼
...
8.2 Startup sequence
	...
	2.	Start OpenClawEngine Gateway (loopback, configured port/token)
	3.	Start OpenClawEngine agent runtime (or openclaw-engine agent-style runner as per upstream)
...
OpenClawEngine supports “any model”, and is configured via its model provider system (docs referenced from README).  ￼
Your implementation must:
	•	map Scrappy’s “selected model” into OpenClawEngine’s model configuration (base URL + model id)
	•	ensure it points to your llama.cpp OpenAI-compatible endpoint

Spec requirements
	•	llama.cpp bind loopback only
	•	stable base URL for the OpenClawEngine process (http://127.0.0.1:<port>/v1)
...
OpenClawEngine security docs highlight:
	•	bind exposure risks
	•	mandatory auth
...
OpenClawEngine protocol supports approvals:
...
OpenClawEngine supports running tool execution in Docker to reduce blast radius.  ￼
Plan
...
11.1 Upstream repositories
	•	Core assistant repo (OpenClawEngine): openclaw-engine/openclaw-engine  ￼
	•	Website / installer scripts: openclaw-engine/molt.bot  ￼
	•	Docs: docs.molt.bot (protocol, security, channels, configuration)  ￼
	•	Org repos (extras like Lobster workflows): openclaw-engine org page lists related repos  ￼
	•	Optional community skill index: “awesome openclaw-engine skills” (for future extension UI)  ￼

11.2 Runtime dependencies (bundled with Scrappy)
	•	Node.js ≥ 22 (required by OpenClawEngine)  ￼
	•	OpenClawEngine JS distribution (your packaging choice):
...
(These are implementation suggestions; the critical “dependency” is the upstream OpenClawEngine WS protocol and config schema.)  ￼
...
12.2 Integration tests (desktop)
	...
	•	Slack DM → reply
	•	Telegram DM → reply
	•	Restart Scrappy, confirm sessions persist (OpenClawEngine stores sessions under state dir)  ￼
...
14) What you should not do (common pitfalls)
	...
	•	Don’t rely on OpenClawEngine onboarding wizard in production packaging; generate config directly.
	•	Don’t share Scrappy’s tool permissions or filesystem access with OpenClaw mode—keep it sandboxable and auditable.
	•	Don’t implement your own Slack/Telegram connectors; let OpenClawEngine own the surfaces (that’s the whole point).  ￼


Below is a Rust-first WS client blueprint you can drop into Scrappy Inference for the standalone OpenClaw mode:
...
This is designed to be resilient even if OpenClawEngine evolves, by parsing most payloads as serde_json::Value and normalizing.
...
This is the shape you want in a standalone module, e.g. openclaw_ws.rs.
...
Background task
	•	spawn one OpenClawWsClient::run_forever() task
	•	ui_tx is an mpsc::Sender<UiEvent>
	•	a separate forwarder pushes events into Tauri webview via app_handle.emit_all("openclaw:event", ui_event_json)

Commands

Expose Tauri commands like:
	•	openclaw_list_sessions()
	•	openclaw_chat_history(session_key, before?)
	•	openclaw_send(session_key, text, deliver=false)
	•	openclaw_abort(session_key, run_id?)
...
Here’s a ready-to-paste Slack App Manifest for Socket Mode that’s appropriate for a local-first OpenClaw/OpenClawEngine-style assistant in Scrappy Inference (Slack + Telegram first). It includes the typical bot scopes and event subscriptions you’ll want for DMs + channel mentions + reactions, plus a /clawd slash command.
