...
> [!NOTE]
> **Architecture update (2026-03-15):** The OpenClaw Node.js sidecar referenced in the original spec has been **fully replaced** by **IronClaw** — a Rust-native in-process library (done 2026-02-28). §2, §7, and §11 have been updated to reflect the current architecture. The MCP tool architecture, sandbox, skills, and code execution concepts remain valid and are implemented in `backend/scrappy-mcp-tools/`.
...
│                                                                      │
│  ┌─────────────────┐  ┌──────────────────┐  ┌───────────────────┐  │
│  │ Standard LLM     │  │  Rig Agent       │  │  IronClaw         │  │
│  │ Inference         │  │  (Rust)          │  │  (in-process      │  │
│  │ (llama.cpp)      │  │  Custom rig tools│  │   Rust library)   │  │
│  └────────┬─────────┘  └────────┬─────────┘  └────────┬──────────┘  │
│           │                      │                      │            │
│           └──────────────────────┼──────────────────────┘            │
│                                  │                                    │
...
| :--- | :--- | :--- | :--- |
| **Rig Agent** | Rust | Custom rig tools + Rhai sandbox | Via `scrappy-mcp-tools` |
| **IronClaw** | Rust (in-process) | Built-in tool system | IronClaw skills system |
...
5. Direct API interfaces so IronClaw can call the same Rust tools
...
│             │              │              │                           │
│  ┌──────────▼──────┐ ┌────▼──────────┐ ┌▼────────────────────┐     │
│  │ Rig Agent       │ │ IronClaw      │ │ Standard LLM        │     │
│  │ (direct Rust)   │ │ (in-process)  │ │ (via tool routing)  │     │
│  └─────────────────┘ └───────────────┘ └─────────────────────┘     │
└──────────────────────────────────────────────────────────────────────┘
                                   │
                            MCP Client (Rust)
...
#### IronClaw (In-Process Rust Library)

IronClaw runs in the same process as the Tauri backend. It accesses `scrappy-mcp-tools` directly as a Rust dependency — no IPC overhead:

```rust
// IronClaw uses scrappy-mcp-tools directly (same process)
use scrappy_mcp_tools::{McpClient, discovery::search_tools};

// Progressive discovery
let categories = search_tools(&client, "", DetailLevel::Categories).await?;

// Call a specific tool
let gold_price = finance::get_stock_price(&client, "GLD").await?;

// Execute a code-mode script (runs in Rhai sandbox)
let result = sandbox.execute(r#"
    let prices = get_stock_price_batch(["AAPL", "GOOGL", "MSFT"]);
    let total = prices.len();
    #{ total_symbols: total, prices: prices }
"#).await?;
```
...
## 7. IronClaw Integration (In-Process Rust Library)

> **Update (2026-03-15):** The Node.js sidecar (`openclaw-engine/`) was deleted during IronClaw migration (Phase 5, 2026-02-28). IronClaw runs in-process — no IPC bridge needed. It accesses `scrappy-mcp-tools` directly as a Rust library dependency.

### 7.1 Direct Rust API Access

IronClaw calls Rust tools directly — no WebSocket IPC or Tauri invoke needed:

```rust
// backend/src/rig_lib/sandbox_factory.rs
...
```rust
// In IronClaw agent loop (backend/src/openclaw/ironclaw_bridge.rs)

use scrappy_mcp_tools::{McpClient, sandbox::Sandbox, skills::SkillManager};

// Progressive discovery (direct Rust API call)
let categories = search_tools(&client, "", DetailLevel::Categories).await?;

// Direct tool call (simple queries)
let gold_price = finance::get_stock_price(&client, "GLD").await?;

// Code-mode execution (Rhai sandbox — same process)
let result = sandbox.execute(&script).await?;

// Skill execution
let report = skill_manager.execute_skill("precious_metals_report", params, &sandbox).await?;
```
```
...
- [x] `scrappy-mcp-tools` crate skeleton — `backend/scrappy-mcp-tools/`
...
- [x] Added as path dependency to main Tauri app — `backend/Cargo.toml`
...
- [x] **Agent Skill Creation**: Allow agents to persist new skills (`save_skill` + direct API).
...
## 11. IronClaw Migration — DONE ✅

> **Completed 2026-02-28.** The Node.js OpenClaw sidecar has been fully replaced by IronClaw, a Rust-native in-process library. See `tauri_dropin_spec.md` for the full 8-phase migration spec.

### What was done:

- [x] **Crate setup**: IronClaw runs as an in-process Rust library (not a sidecar binary).
- [x] **Direct MCP client**: IronClaw uses `scrappy-mcp-tools` directly as a Rust dependency.
- [x] **Host bridge**: IronClaw accesses Scrappy’s tools via the Tauri `AppHandle` (same process).
- [x] **Agent loop**: Rust-based agent loop with 110+ Tauri commands migrated from WS RPC.
- [x] **Sandbox**: IronClaw uses `scrappy-mcp-tools/src/sandbox.rs` for code execution.
- [x] **Dead code removed**: `ws_client.rs`, `ipc.rs`, `openclaw-engine/` directory (625 MB) all deleted.
...
## Appendix B: IPC Bridge — REMOVED

> **Update (2026-03-15):** The WebSocket IPC bridge (`ws_client.rs`, `ipc.rs`, `McpRequestHandler`) was deleted during the IronClaw migration (Phase 5, 2026-02-28). IronClaw runs in-process as a Rust library — it accesses `scrappy-mcp-tools` directly via Rust API calls. No IPC protocol is needed.
>
> The original Appendix B documented the `mcp.list_tools` and `mcp.call_tool` WebSocket RPC methods that bridged the Node.js sidecar to the Rust backend. This is no longer relevant.
...
```rust
// backend/src/rig_lib/orchestrator.rs (actual implementation)
...
The `Orchestrator` in `backend/src/rig_lib/orchestrator.rs` supports **two execution modes** that can be toggled per-instance:
...
```
$ cd backend/scrappy-mcp-tools && cargo test
running 17 tests
