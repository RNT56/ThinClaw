...
| Canvas hosting | ✅ | 🚧 | `CanvasTool` (621 LOC, [`src/tools/builtin/canvas.rs`](src/tools/builtin/canvas.rs)) — Show/Update/Dismiss/Notify actions implemented; not yet registered in ToolRegistry at startup |
...
| Video support | ✅ | 🚧 | P3 | `VideoAnalyzer` stub (302 LOC, [`src/media/video.rs`](src/media/video.rs)) — metadata extraction, extension detection; keyframe/audio extraction pending ffmpeg |
...
| Voice wake | ✅ | 🚧 | Partial | VAD-based voice activation with recording overlay. Lacks wake-word engine. |
...
| Canvas system (A2UI) | ✅ | 🚧 | P3 | `CanvasTool` backend done ([`src/tools/builtin/canvas.rs`](src/tools/builtin/canvas.rs)); needs ToolRegistry registration + web gateway asset hosting |
...
| OAuth flows | ✅ | 🚧 | Generic OAuth plumbing; provider-specific flows incomplete |
...
| Voice wake (VAD) | `VoiceWakeOverlay.tsx` + `use-voice-wake.ts` — VAD energy detection → auto-record → transcribe | 🚧 Partial — lacks wake-word engine |
...
| **BridgedTool auto-registration** | P5 | Sensor tools auto-appear in ToolRegistry — may need UI refresh |
...
| 14 | **Voice wake** (VAD-based) | Voice activation with recording overlay | 1 day (VAD) | 🚧 Partial |
...
- 🚧 Video support (`VideoAnalyzer` stub — metadata extraction works, ffmpeg keyframe/audio pending; [`src/media/video.rs`](src/media/video.rs))
...
- ❌ CLI: `logs` query/filter
- ❌ CLI: `update` self-update
- ❌ CLI: `/subagents spawn` from chat
- ❌ CLI: `/export-session` transcript export
...
**Media**
- 🚧 Video keyframe extraction — `VideoAnalyzer` exists ([`src/media/video.rs`](src/media/video.rs)); basic metadata works, ffmpeg keyframes pending
- ❌ Media caching layer
- ❌ Incremental TTS playback
- ❌ Telegram sticker-to-image conversion

**UI & Control**
- 🚧 Canvas system (A2UI) — `CanvasTool` implemented ([`src/tools/builtin/canvas.rs`](src/tools/builtin/canvas.rs), 621 LOC); needs ToolRegistry registration + gateway hosting
...
- 🚧 iMessage channel — code exists ([`src/channels/imessage.rs`](src/channels/imessage.rs), 445 LOC), config resolves; needs wiring into main.rs startup
...
- ❌ `BridgedTool` → `Tool` trait adapter (bridge infrastructure done, not in ToolRegistry)
...
- 🚧 Voice wake — VAD-based energy detection + recording overlay + auto-transcribe. Missing: wake-word engine (Porcupine/Snowboy)
...
### Deferred (No Urgency)
- 🔮 Sherpa-ONNX keyword spotting (scaffold exists, needs ML models)
