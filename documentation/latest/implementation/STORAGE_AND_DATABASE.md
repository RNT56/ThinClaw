...
11. [Configuration Storage](#11-configuration-storage)
    ...
    - 11.4 [Auth Profiles *(Removed)*](#114-auth-profiles-removed)
...
**Consumers:**
- **IronClaw engine** — reads granted keys via `KeychainSecretsAdapter` (implements `SecretsStore` trait)
...
**Security Note:** `snapshot()` method was intentionally removed to prevent unfiltered key access. IronClaw only receives keys that have been explicitly granted via per-provider `_granted` flags.
...
Generated configuration file consumed by the IronClaw agent engine (in-process):
...
### 11.4 Auth Profiles *(Removed)*

> **Architecture change (2026-02-28):** `auth-profiles.json` is **no longer generated**. The former `write_config()` code that produced this file (~292 LOC) was removed during the IronClaw migration. IronClaw reads API keys **directly** from macOS Keychain via the `KeychainSecretsAdapter`, eliminating the on-disk plaintext shadow. See `engine.rs` L391-394 for the deletion note.
...
│   └── sessions/             ← OpenClaw chat session state
...
OpenClaw sessions are managed by the IronClaw agent engine (in-process Rust library):

- **Sessions:** Managed by IronClaw's `ironclaw::api::sessions` module backed by libSQL
- **Memory:** Agent workspace files (SOUL.md, MEMORY.md, etc.) managed via `ironclaw::api::memory`
- **Accessed in-process:** Scrappy communicates with IronClaw via direct Rust function calls through `rpc.rs` Tauri commands — no WebSocket or IPC overhead
...
| **~~Auth-profiles.json~~** | *(Removed)* — Previously contained plaintext API keys; eliminated during IronClaw migration. IronClaw reads keys directly from Keychain. | No longer applicable |
