...
| `rig_lib/agent.rs` | 281 L | `RigManager` struct — the agent wrapper |
...
| `rig_lib/orchestrator.rs` | 935 L | `Orchestrator` — full ReAct loop, summarisation, MCP |
...
| `rig_lib/sandbox_factory.rs` | 540 L | `create_sandbox()` — Rhai engine with all tools wired |
...
| `rig_lib/tools/calculator_tool.rs` | 801 L | `CalculatorTool` + `evaluate()` — hand-written recursive-descent parser |
...
| `chat.rs` | 784 L | Primary consumer: `chat_stream`, `chat_completion`, `count_tokens` |
