...
## Tauri Commands (110 total — `rpc.rs` 84, `sessions.rs` 18, `gateway.rs` 8)

> This document covers the most important commands and their response shapes.
> For the full list, see `backend/src/openclaw/commands/`.
...
> **Note:** Most items from the original list have since been wired. Only 2 remain:
...
**Previously listed but now wired:**
- ~~Hook registry~~ → `openclaw_hooks_list` (Sprint 13)
- ~~Skill list~~ → `openclaw_skills_list` (Phase 3)
- ~~Log stream~~ → `openclaw_logs_tail` (Phase 6)
- ~~`CachedResponseStore`~~ → `openclaw_cache_stats` (Sprint 14)
