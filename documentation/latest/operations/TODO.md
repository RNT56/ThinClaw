...
- [x] **Remove vestigial Node.js sidecar** *(done 2026-03-13)*  
  `bin/node` was bundled as a Tauri externalBin but never spawned — leftover from the Node.js OpenClaw engine replaced by IronClaw (in-process Rust library).  
  **Removed:**  
  - `backend/scripts/download_node.js` — deleted  
  - `backend/tauri.conf.json` — removed `"bin/node"` from `externalBin`  
  - `scripts/generate_tauri_overrides.sh` — removed `"bin/node"` from all 4 engine variants  
  - `package.json` — removed `"setup:node"` script and from `"setup:all"` chain  
  - `backend/bin/node-*` — deleted (~112 MB reclaimed)
...
- [x] **Cache `RigManager` across requests** *(done 2026-02-22)*  
  ...
  → `backend/src/rig_cache.rs`, `backend/src/chat.rs`, `backend/src/lib.rs`
...
- [x] **① Deharcode `ToolRouter` host-tool dispatch** *(done 2026-02-22)*  
  ...
  → `backend/src/rig_lib/tool_router.rs`
...
- [x] **② Wire tool events from `McpRequestHandler` to OpenClaw engine** *(done 2026-02-22)*  
  ...
  → `backend/src/openclaw/` (ipc.rs since deleted during IronClaw migration)
...
- [x] **③ Unify legacy `<tool_code>` XML loop with MCP sandbox loop** *(done 2026-02-22)*  
  ...
  → `backend/src/rig_lib/orchestrator.rs`
...
- [x] **Implement TTS sidecar** *(done 2026-02-22)*  
  Implemented Piper-based TTS sidecar in `backend/src/tts.rs`. `tts_synthesize` Tauri command spawns
  ...
  Binary must be bundled at `backend/bin/piper` — see `documentation/latest/MICROSERVICES_AND_SIDECARS.md`.  
  → `backend/src/tts.rs`, `backend/src/sidecar.rs`, `frontend/src/components/chat/MessageBubble.tsx`
...
- [x] **Add runtime null guards over `bindings.ts`** *(done 2026-02-22)*  
  ...
  Created `frontend/src/lib/guards.ts` with `defined()`, `withDefault()`, and `unwrapResult()` helpers.  
  Fixed the `TAURI_CHANNEL` name collision (generated placeholder vs. runtime import).  
  → `frontend/src/lib/bindings.ts`, `frontend/src/lib/guards.ts`, `backend/src/lib.rs`
...
- [x] **Lazy-load heavy settings tabs** *(done 2026-02-22)*  
  ...
  → `frontend/src/components/settings/SettingsPages.tsx`
...
- [x] **Make `ToolRegistryCache` TTL configurable** *(done 2026-02-22)*  
  ...
  → `backend/src/config.rs`, `backend/scrappy-mcp-tools/src/discovery.rs`
...
- [x] **🟠 Risk #3: No timeout on WS RPC calls — can hang forever** *(fixed 2026-02-24)*  
  ...
  → `backend/src/openclaw/ws_client.rs` (since deleted during IronClaw migration — risk eliminated)
...
- [x] **🟠 Risk #5: auth-profiles.json persists plaintext API keys on disk** *(fixed 2026-02-24)*  
  ...
  → `backend/src/openclaw/config/engine.rs`, `backend/src/openclaw/commands/gateway.rs`
...
- [x] **🟡 Arch #7: Duplicated session delete retry logic** *(fixed 2026-02-24)*  
  ...
  → `frontend/src/components/openclaw/OpenClawChatView.tsx`
...
- [x] **🟡 Arch #8: Verbose `info!()` logging on every WS message** *(fixed 2026-02-24)*  
  ...
  → `backend/src/openclaw/ws_client.rs` (since deleted during IronClaw migration)
...
- [x] **🟡 Arch #9: Fleet polls every 3s unconditionally** *(fixed 2026-02-24)*  
  ...
  → `frontend/src/components/openclaw/fleet/FleetCommandCenter.tsx`
...
- [x] **🟡 Arch #11: No backpressure on UI event channel (64-deep mpsc)** *(fixed 2026-02-24)*  
  ...
  → `backend/src/openclaw/commands/gateway.rs`
...
- [ ] **S-Perf #9: Imagine search uses `LIKE %query%` (full table scan)** *(accepted — won't fix)*  
...
- [x] **S-Arch #13: `auth-profiles.json` contains plaintext API keys** *(mitigated 2026-02-24)*  
  Already mitigated: chmod 600 on write + deleted on gateway stop. Remaining risk (file readable by same-user processes while gateway is active) is accepted — full elimination would require IronClaw to accept keys via env vars/stdin instead of file.  
  → `backend/src/openclaw/config/engine.rs`
...
- [x] **Phase 8: Sidecar cleanup** — Deleted `openclaw-engine/` directory (625 MB of Node.js)
