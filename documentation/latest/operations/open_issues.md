...
### IC-007 · ~47 `unsafe { std::env::set_var() }` calls during async operation
...
Leveraged IronClaw's existing `INJECTED_VARS` overlay pattern (used for secrets injection) to create a new `BRIDGE_VARS` overlay — a thread-safe `LazyLock<RwLock<HashMap>>` that `optional_env()` checks **before** secrets and real env vars.

**Changes across 6 files:**
- `ironclaw/src/config/mod.rs` — Added `BRIDGE_VARS` static, `inject_bridge_vars()`, `remove_bridge_vars()`, `clear_bridge_vars()`, `bridge_var_exists()` public API
- `ironclaw/src/config/helpers.rs` — Updated `optional_env()` to check `BRIDGE_VARS > INJECTED_VARS > std::env::var`
- `ironclaw/src/tools/builtin/shell.rs` — Migrated `is_safe_bins_only()` from `std::env::var()` to `optional_env()`
- `ironclaw/src/talk_mode.rs` — Migrated `WHISPER_HTTP_ENDPOINT` lookup to `optional_env()`
- `ironclaw/src/media/audio.rs` — Migrated `WHISPER_HTTP_ENDPOINT` lookup to `optional_env()`
- `backend/src/openclaw/ironclaw_bridge.rs` — Replaced all ~47 `set_var`/`remove_var` calls with HashMap + `inject_bridge_vars()`; `stop()` now uses `clear_bridge_vars()`

**Result:** Zero `unsafe` blocks remain in `ironclaw_bridge.rs`. All 2484 tests pass. Build compiles with zero errors and zero warnings.
...
| ID | Severity | Short Title | Solution | Status |
|----|----------|-------------|----------|--------|
| IC-001 | 🔴 Critical | `auto_approve_tools` not refreshed | Clear env var in `stop()`, unconditional `set_var` in `start()` | ✅ DONE |
| IC-002 | 🔴 Critical | Concurrent workspace writes | Per-user `RwLock` in `SessionManager` | ✅ DONE |
| IC-003 | 🔴 Critical | Notification forwarder leaks | Track `JoinHandle` in `BackgroundTasksHandle` | ✅ DONE |
| IC-004 | 🔴 Critical | `hasFailures` false positives | Remove string heuristics, use `metadata.status` only | ✅ DONE |
| IC-005 | 🔴 Critical | `extractSessionKey()` silent failure | Add regex fallback + `console.warn` | ✅ DONE |
| IC-006 | 🔴 Critical | Zombie RoutineRun DB records | Add zombie reaper in cron ticker | ✅ DONE |
| IC-007 | 🟠 High | Unsafe `set_var()` during async | Bridge overlay (`BRIDGE_VARS`) replaces all `set_var` | ✅ IMPLEMENTED |
| IC-008 | 🟠 High | Silent LLM fallback | Emit `UiEvent::Error` warning toast | ✅ DONE |
| IC-009 | 🟠 High | Raw `__tauri__` invoke | Replace with `commands.openclawRevealFile()` | ✅ DONE |
| IC-010 | 🟠 High | Stateful RegExp `lastIndex` | Use non-global regex for test, fresh regex for exec | ✅ DONE |
| IC-011 | 🟠 High | 5-min reconciliation window | Tighten to 30 seconds | ✅ DONE |
| IC-012 | 🟠 High | `RigManager` panics on None | Conditional tool registration with `if let Some` | ✅ DONE |
| IC-013 | 🟠 High | Duplicated heartbeat prompt | Extract `build_daily_context()` function | ✅ DONE |
| IC-014 | 🟡 Medium | `coreTab` initial state inverted | Always initialize to `'chat'` | ✅ DONE |
| IC-015 | 🟡 Medium | Editor commentary in source | Replace with proper doc comment | ✅ DONE |
| IC-016 | 🟡 Medium | Hardcoded `"chatml"` family | Read from config, infer from model name | ✅ DONE |
| IC-017 | 🟡 Medium | `SkillInstallTool` schema lies | Use `anyOf` schema | ✅ DONE |
| IC-018 | 🟡 Medium | `spawn_fire()` drops JoinHandle | Use `JoinSet` for tracking | ✅ DONE |
| IC-019 | ~~🟡~~ | ~~Session pruning hardcoded~~ | **RETRACTED** | N/A |
| IC-020 | 🟡 Medium | Event listener cleanup race | Add `isMounted` ref guard | ✅ DONE |
| IC-021 | 🟡 Medium | Identity keys not zeroized | Explicit `zeroize_keys()` method | ✅ DONE |
| IC-022 | ~~🟡~~ | ~~`MetaConfig` never populated~~ | **RETRACTED** | N/A |
| IC-023 | 🔵 Low | `rag_chat()` dead code | Mark `#[deprecated]` | ✅ DONE |
| IC-024 | 🔵 Low | `pendingOptimisticMessages` unused | Remove ref and push | ✅ DONE |
| IC-025 | 🔵 Low | Job monitor drop unexplained | Add inline comment | ✅ DONE |
| IC-026 | 🔵 Low | Double registry lock | Combine into single acquisition | ✅ DONE |
| IC-027 | ~~🔵~~ | ~~`node_host_enabled` unused~~ | **RETRACTED** | N/A |
| IC-028 | 🔵 Low | `isUserScrolling` inverted | Rename to `isAutoScrollPinned` | ✅ DONE |
| IC-029 | 🔵 Low | Nested JSON regex broken | Use greedy match to EOL | ✅ DONE |
| IC-030 | 🔵 Low | Safari animation flicker | Add `maxHeight: 600` | ✅ DONE |
| IC-031 | 📦 Legacy | Deprecated Ansible docs | Delete section | ✅ DONE |
| IC-032 | 📦 Legacy | `default_dm_policy()` visibility | Change to `fn` | ✅ DONE |
| IC-033 | 📦 Legacy | Hardcoded "OpenClaw" name | Read from `AGENT_NAME` env var | ✅ DONE |
| IC-034 | 📦 Legacy | Dead `coreTab` for non-core | Addressed by IC-014 | ✅ DONE |
| DD-001 | 📋 Docs | Hard-coded date | Replace with git log reference | ✅ DONE |
| DD-002 | 📋 Docs | `mod.rs` undo caveat missing | Add remote-proxy note | ✅ DONE |
| DD-003 | 📋 Docs | `heartbeat.rs` doc stale | Update module doc | ✅ DONE |

### Summary

- **Resolved:** 31 of 31 actionable items (28 code + 3 docs)
- **Retracted:** 3 items (IC-019, IC-022, IC-027 — invalid findings)
- **Total:** 34 issues audited, 31 resolved, 3 retracted, **0 open**
- **Build verification:** ✅ `cargo check` + ✅ `tsc --noEmit`

### Key Implementation Notes

| Issue | Note |
|-------|------|
| IC-021 | Changed from `Drop` impl to explicit `zeroize_keys()` method — `Drop` prevents struct moves in 16+ places |
| IC-006 | Uses existing `cleanup_stale_routine_runs()` DB method. Reaper runs on every cron tick |
| IC-018 | `spawn_fire()` changed to `async fn` for `JoinSet::spawn()` lock acquisition |
| IC-003 | `notification_forwarder_handle` threaded out via 3-tuple return from routine setup |
