...
│   │
│   └── voice/
│       └── VoiceWakeOverlay.tsx  # Wake-word detection overlay
│
├── hooks/
...
│   ├── use-cloud-models.ts    # Cloud provider model listing
│   ├── use-cloud-status.ts    # Cloud provider status monitoring
│   ├── use-inference-backends.ts # Inference backend management
│   ├── use-voice-wake.ts      # Wake-word detection ("Hey Scrappy")
│   └── useEngineSetup.ts      # Engine first-launch setup state
│
└── lib/
...
| `openclaw-event` | `openclaw/ironclaw_channel.rs` | `OpenClawChatView` |
