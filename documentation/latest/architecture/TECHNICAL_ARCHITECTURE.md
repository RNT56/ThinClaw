...
5. [IronClaw Agent Engine (In-Process)](#5-ironclaw-agent-engine-in-process)
   - 5.1 [Module Structure](#51-module-structure)
   - 5.2 [IronClawState — Engine Lifecycle](#52-ironclawstate--engine-lifecycle)
   - 5.3 [TauriChannel](#53-taurichannel)
   - 5.4 [SecretsStore Adapter](#54-secretsstore-adapter)
   - 5.5 [Commands — Direct API Calls](#55-commands--direct-api-calls)
   - 5.6 [Config Persistence](#56-config-persistence)
   - 5.7 [Fleet & Remote Deploy](#57-fleet--remote-deploy)
6. [Native Rig Agent (`backend/src/rig_lib/`)](#6-native-rig-agent)
...
|------|--------|-------------|
| **OpenClaw** | IronClaw in-process Rust library | Full autonomous agentic loops, skill ecosystem, HITL approval, cron jobs |
...
│   ├── ironclaw/               # IronClaw agent engine (Rust crate, linked in-process)
...
`backend/src/lib.rs` (461 lines) contains the `run()` function which:
...
| `IronClawState` | `openclaw/ironclaw_bridge.rs` | In-process IronClaw agent engine lifecycle |
...
> **Note:** The former `node` sidecar entry (OpenClaw engine runner) was removed — IronClaw now runs **in-process** as a linked Rust crate.
...
| `mcp_base_url` | `Option<String>` | `None` | Remote MCP server URL (read directly from `UserConfig` in `chat.rs`) |
...
Migrations live in `backend/migrations/` (10 total, chronological):
...
| `OpenClaw/openclaw.json` | OpenClaw engine runtime config |
...
`process_tracker.rs` solves the **orphan process problem**: if Scrappy crashes, previously spawned `llama-server` / `whisper-server` processes would continue running and consuming resources.
...
Dynamic libraries (`.dylib`) and Metal shaders (`.metal`) are bundled as resources.
...
| `openclaw-event` | `UiEvent` | `openclaw/ironclaw_channel.rs` |
...
└──────────┬────────────────┘
           ...
           ▼
┌───────────────────────────┐
│  IronClaw Agent (in-proc)  │          ← In-process Rust crate
│  KeychainSecretsAdapter    │          ← Reads via SecretsStore trait
│  (only granted keys)       │
└───────────────────────────┘
```

> **Note:** The former intermediate `auth-profiles.json` file was removed during the IronClaw migration. IronClaw now reads secrets **directly** from macOS Keychain via the `KeychainSecretsAdapter`, eliminating the on-disk plaintext shadow.
...
> **Removed:** `npm run setup:node` and `npm run setup:openclaw-engine` — no longer needed since IronClaw runs in-process.
...
| **Piper binary not auto-downloaded** | `backend/bin/piper` | The `download_ai_binaries.js` script does not yet fetch Piper; it must be placed manually. |
