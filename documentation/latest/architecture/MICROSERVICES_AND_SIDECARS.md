...
10. [Build-time Infrastructure Scripts](#10-build-time-infrastructure-scripts)
    ...
    - 10.2 [~~download_node.js~~ (Removed)](#102-download_nodejs-removed)
...
The `deploy-remote.sh` script and `Dockerfile` are retained for deploying IronClaw to remote Linux servers via Ansible + Docker. The desktop app can connect to remote instances via Tailscale.
...
### 10.2 ~~download_node.js~~ (Removed)

> [!NOTE]
> This script has been removed. Node.js is no longer bundled as a sidecar — the OpenClaw agent engine (IronClaw) now runs as an in-process Rust library linked directly into the Tauri binary.
...
> **Note:** The `openclaw-engine` gateway (port `18789`) was removed on 2026-02-28 when IronClaw was embedded in-process. See §7 for details.
...
All tokens are generated at process start (not stored persistently) and re-generated on each restart. The llama-server token is passed to the IronClaw engine config so both sides have matching credentials.
...
[User triggers OpenClaw agent task]
         │
         ▼
[Rust: openclaw/commands → ironclaw::api::chat::send_message()]
         │
         ▼
[IronClaw Agent (in-process) runs turn in background tokio task]
         │
         ├─ agent processes turn, calls tools (fs, web, messaging, etc.)
         │
         ├─ agent → TauriChannel events (StreamChunk, ToolCall, ApprovalRequest, ...)
         │
         ▼
[TauriChannel → UiEvent → app.emit("openclaw-event", ui_event)]
         │
         ▼
...
### Replacing the IronClaw agent engine
IronClaw is an in-process Rust library (`ironclaw` crate) linked via `backend/Cargo.toml`. All agent commands call `ironclaw::api::*` directly. To replace it, implement the same API surface and update `ironclaw_bridge.rs` to construct your alternative agent. The frontend consumes only `UiEvent` variants emitted via `TauriChannel` — the UI contract is agent-engine-agnostic.
...
**Current status:** `mcp_base_url` and `mcp_auth_token` are stored in `UserConfig` and read directly from `ConfigManager` in `chat.rs`. The `GatewayTab` settings page exposes UI fields that write these values to `UserConfig`. No code changes are needed to point at a custom MCP server — configure the URL and token in the Gateway settings tab.

---

### 8.2 McpRequestHandler — Reverse-RPC Entry Point

**File:** `backend/src/openclaw/commands/rpc.rs`, `scrappy-mcp-tools/`

IronClaw's tool system calls MCP tools directly via the `scrappy-mcp-tools` crate, without any WebSocket intermediary. When the agent needs a tool executed, it calls through the `ToolBridge` which dispatches to the appropriate handler:
...
This is the **bridge between the IronClaw agent and the Rust tool ecosystem** — the agent calls tools directly via this in-process handler.

**`McpOrchestratorConfig`** (resolved per-request in `chat.rs`):
...
pub struct McpOrchestratorConfig {
    ...
    pub sandbox_enabled:      bool,              // Always true when mcp_base_url is set
...
- By the **McpRequestHandler** (`openclaw/commands/rpc.rs`) for IronClaw tool calls
