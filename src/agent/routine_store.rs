//! Root database adapter for the extracted agent routine-store port.
...
impl RootRoutineStorePort {
    pub fn shared(store: Arc<dyn Database>) -> Arc<dyn RoutineStorePort> {
        Arc::new(Self { store })
    }
}
...
mod tests {
    ...
    #[test]
    fn routine_store_adapter_is_object_safe() {
        fn assert_object_safe(_: Option<Arc<dyn RoutineStorePort>>) {}
