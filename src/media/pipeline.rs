//! Media extraction pipeline glue.
//!
//! `MediaPipeline` stays root-local because its default extractor set
//! includes [`AudioExtractor`](super::audio::AudioExtractor), which depends on
//! root `crate::config` for endpoint resolution and therefore cannot move into
//! `thinclaw-media` without creating a `media → config → channels → media`
//! dependency cycle. The image, PDF, and document extractors plus the
//! [`MediaExtractor`] trait and [`MediaExtractError`] all live in
//! `thinclaw-media`.

use super::types::{MediaContent, MediaExtractError, MediaExtractor};
