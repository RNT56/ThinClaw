...
impl ProviderPresetConfig {
    ...
    pub fn from_env(preset: ProviderPreset) -> Result<Self, String> {
        let base_url = optional_env(preset.base_url_env())
            .map_err(|e| format!("Failed to read {}: {}", preset.base_url_env(), e))?
            .unwrap_or_else(|| preset.default_base_url().to_string());

        let api_key = optional_env(preset.api_key_env())
            .map_err(|e| format!("Failed to read {}: {}", preset.api_key_env(), e))?
            .map(SecretString::from);

        let model = optional_env(preset.model_env())
            .map_err(|e| format!("Failed to read {}: {}", preset.model_env(), e))?
...
pub fn detect_preset() -> Option<ProviderPreset> {
    for preset in ProviderPreset::all() {
        if let Ok(Some(_)) = optional_env(preset.api_key_env()) {
            return Some(preset);
        }
    }
    None
