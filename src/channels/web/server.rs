...
async fn learning_history_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_artifact_versions_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_feedback_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_feedback_submit_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn learning_provider_health_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_code_proposals_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_code_proposal_review_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn learning_rollbacks_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<learning_api::LearningListQuery>,
...
async fn learning_rollback_submit_handler(
    State(state): State<Arc<GatewayState>>,
...
fn research_gpu_cloud_info(
    ...
    Some(experiments_api::ExperimentGpuCloudProviderInfo {
        ...
        secret_name: crate::experiments::adapters::gpu_cloud_secret_name(backend)
            ...
            .to_string(),
        connected,
        template_hint: Some(crate::experiments::adapters::gpu_cloud_template_hint(backend)),
    })
}
...
> {
    ...
    let image_or_runtime = req
        ...
        .filter(|value| !value.is_empty())
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| crate::experiments::adapters::default_research_runner_image().to_string());
    let (backend_config, warnings, launch_payload_preview) =
        if backend == crate::experiments::ExperimentRunnerBackend::Lambda {
            ...
            let (backend_config, warnings) =
                crate::experiments::adapters::build_lambda_backend_config(
...
async fn experiments_projects_list_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_project_detail_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_project_create_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_project_update_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_project_delete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiments_runners_list_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_runner_detail_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_runner_create_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_runner_update_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_runner_delete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiments_runner_validate_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_campaign_start_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_campaigns_list_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_campaign_detail_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_campaign_pause_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_campaign_resume_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<experiments_api::ExperimentCampaignActionResponse>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiments_campaign_cancel_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<experiments_api::ExperimentCampaignActionResponse>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiments_campaign_promote_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<experiments_api::ExperimentCampaignActionResponse>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiments_trials_list_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_trial_detail_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_artifacts_list_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_targets_list_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_target_create_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_target_link_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_target_update_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
...
async fn experiments_target_delete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
    ...
    if !experiments_api::delete_target(store, &user_id, id)
        ...
        .map_err(experiment_api_error)?
    {
        return Err((StatusCode::NOT_FOUND, format!("Experiment target {id} not found")));
    }
    broadcast_experiment_opportunity_update(
        &state,
        id.to_string(),
...
async fn experiments_model_usage_list_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_opportunities_list_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<ExperimentsLimitQuery>,
...
async fn experiments_gpu_clouds_list_handler(
    State(state): State<Arc<GatewayState>>,
    Query(query): Query<ExperimentsQuery>,
...
async fn experiments_gpu_cloud_connect_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiments_gpu_cloud_validate_handler(
    State(state): State<Arc<GatewayState>>,
    Path(provider): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let backend = research_gpu_cloud_backend(&provider).ok_or((
...
async fn experiments_gpu_cloud_template_handler(
    State(state): State<Arc<GatewayState>>,
    Path(provider): Path<String>,
    ...
    let connected = if let Some(secret_name) =
        crate::experiments::adapters::gpu_cloud_secret_name(backend)
    {
...
async fn experiments_gpu_cloud_launch_test_handler(
    State(state): State<Arc<GatewayState>>,
    Path(provider): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let backend = research_gpu_cloud_backend(&provider).ok_or((
    ...
    let validation_message = crate::experiments::adapters::validate_gpu_cloud_credentials(
        backend,
        secret.expose(),
    )
    .await
...
async fn experiments_campaign_reissue_lease_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<experiments_api::ExperimentCampaignActionResponse>, (StatusCode, String)> {
    let store = experiment_store(&state)?;
...
async fn experiment_lease_job_handler(
    State(state): State<Arc<GatewayState>>,
...
async fn experiment_lease_credentials_handler(
    State(state): State<Arc<GatewayState>>,
    Path(lease_id): Path<String>,
    headers: HeaderMap,
...
async fn experiment_lease_status_handler(
    State(state): State<Arc<GatewayState>>,
    Path(lease_id): Path<String>,
    headers: HeaderMap,
...
async fn experiment_lease_event_handler(
    State(state): State<Arc<GatewayState>>,
    Path(lease_id): Path<String>,
    headers: HeaderMap,
...
async fn experiment_lease_artifact_handler(
    State(state): State<Arc<GatewayState>>,
    Path(lease_id): Path<String>,
    headers: HeaderMap,
...
async fn experiment_lease_complete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(lease_id): Path<String>,
    headers: HeaderMap,
