...
use crate::channels::wasm::schema::WebhookSecretValidation;
        ...
        if is_verify_request {
            ...
            let valid = match auth.secret_validation {
                ...
                WebhookSecretValidation::HmacSha256Body => {
                    webhook_signing::verify_signature(expected.as_bytes(), &body, &provided)
                }
            };
...
/// manager, exchanges the auth code, stores the resulting token, auto-activates
/// the tool, and renders a success or error page for the user to close.
#[allow(dead_code)]
...
mod tests {
    ...
    use crate::channels::wasm::router::{
        RegisteredEndpoint, RegisteredWebhookAuth, WasmChannelRouter,
    };
    ...
    async fn test_get_verify_token_validation() {
        ...
        let app = create_wasm_channel_router(Arc::clone(&router), None);
        let ok = app
            .clone()
            .oneshot(
                Request::builder()
    ...
    async fn test_post_hmac_validation() {
        ...
        let signature = webhook_signing::sign_payload(b"app-secret", &body);
