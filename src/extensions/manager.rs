...
use crate::channels::wasm::{
    RegisteredWebhookAuth, SharedWasmChannel, WasmChannelHostConfig, WasmChannelLoader,
...
use thinclaw_tools::builtin::extension_tools::{
    CombinedInstallError, ExtensionInstallErrorKind, ExtensionInstallOutcome, FallbackDecision,
    ToolExtensionKind,
};
use thinclaw_types::{
...
fn normalize_callback_base(raw: &str) -> Option<String> {
...
impl ExtensionManager {
    ...
    async fn resolve_callback_base_url(&self, browser_origin: Option<&str>) -> Option<String> {
    ...
    async fn check_wasm_tool_auth_status(
        &self,
        name: &str,
    ...
    fn apply_auth_check(result: &mut AuthResult, check: &WasmToolAuthCheck) {
    ...
    async fn broadcast_auth_completed(
        &self,
        extension_name: &str,
        success: bool,
    ...
    async fn take_pending_auth(&self, nonce: &str) -> Option<PendingAuth> {
        self.cleanup_expired_auths().await;
...
fn extension_kind_from_tool_kind(kind: ToolExtensionKind) -> ExtensionKind {
    match kind {
...
fn install_outcome(result: &Result<InstallResult, ExtensionError>) -> ExtensionInstallOutcome {
    match result {
...
fn install_error_kind(error: &ExtensionError) -> ExtensionInstallErrorKind {
    match error {
