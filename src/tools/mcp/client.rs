...
use crate::tools::tool::{ApprovalRequirement, Tool, ToolArtifact, ToolError, ToolOutput};
...
impl McpClient {
    ...
    pub async fn set_logging_level(&self, level: McpLoggingLevel) -> Result<(), ToolError> {
        ...
        let protocol_level = match level {
            McpLoggingLevel::Debug => crate::tools::mcp::protocol::McpLoggingLevel::Debug,
            McpLoggingLevel::Info => crate::tools::mcp::protocol::McpLoggingLevel::Info,
            McpLoggingLevel::Warning => crate::tools::mcp::protocol::McpLoggingLevel::Warning,
            McpLoggingLevel::Error => crate::tools::mcp::protocol::McpLoggingLevel::Error,
        };
        let response = self
...
mod tests {
    ...
    use crate::tools::mcp::config::{McpConfigStore, McpServersFile, save_mcp_servers_to_db};
    ...
    #[tokio::test]
    async fn test_roots_result_reloads_from_persisted_config() {
        let (db, _temp_dir) = test_db().await;
        let mut servers = McpServersFile::default();
        let mut server = McpServerConfig::new("roots-test", "https://example.com/mcp");
        server.capability_policy.roots = true;
        server.roots_grants = vec!["/initial".to_string()];
        servers.upsert(server.clone());
        save_mcp_servers_to_db(db.as_ref(), "default", &servers)
            .await
            .unwrap();

        let client = McpClient::new_configured_with_store(
            server.clone(),
            Some(McpConfigStore::new(Some(db.clone()), "default")),
        );

        let initial = client.runtime.roots_result().await;
        assert_eq!(
            initial["roots"][0]["name"],
            serde_json::Value::String("initial".to_string())
        );

        server.roots_grants = vec!["/updated".to_string()];
        servers.upsert(server);
        save_mcp_servers_to_db(db.as_ref(), "default", &servers)
            .await
            .unwrap();

        let refreshed = client.runtime.roots_result().await;
        assert_eq!(
            refreshed["roots"][0]["name"],
