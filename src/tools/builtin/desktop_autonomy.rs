...
pub struct DesktopAutonomyTool {
    ...
    manager: Arc<DesktopAutonomyManager>,
}

impl DesktopAutonomyTool {
    pub fn apps(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn ui(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn screen(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn calendar_native(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn numbers_native(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn pages_native(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
    ...
    pub fn control(manager: Arc<DesktopAutonomyManager>) -> Self {
        Self {
            ...
            match self.kind {
                ...
                DesktopAutonomyToolKind::Control => match action {
                    "status" => serde_json::to_value(self.manager.status().await)
                        .map_err(|e| format!("failed to serialize autonomy status: {e}")),
                    "pause" => {
                        let reason = params
                            .get("reason")
                    ...
                    "bootstrap" => serde_json::to_value(self.manager.bootstrap().await?)
...
impl Tool for DesktopAutonomyTool {
    ...
    fn execution_timeout(&self) -> Duration {
        Duration::from_secs(self.manager.config().desktop_action_timeout_secs.max(30))
    }
...
mod tests {
    ...
    fn schemas_expose_action_enum() {
        let manager = Arc::new(DesktopAutonomyManager::new(
            crate::config::DesktopAutonomyConfig {
                ...
                desktop_action_timeout_secs: 60,
                capture_evidence: true,
                emergency_stop_path: std::path::PathBuf::from("/tmp/autonomy-stop"),
