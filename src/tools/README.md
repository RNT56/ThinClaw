...
5. Install with `thinclaw tool install path/to/tool.wasm`
...
When running `thinclaw tool auth <tool>`:
...
Both are first-class in the extension system (`thinclaw tool install` handles both), but they have different strengths.

**WASM Tools (ThinClaw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, ThinClaw can't prevent leaks
