...
impl<'a> WasmToolOAuthFlow<'a> {
    ...
    pub async fn combined_oauth_config(
        ...
        let builtin = crate::cli::oauth_defaults::builtin_credentials(&canonical_secret_name(
            &auth.secret_name,
        ));
...
pub fn resolve_oauth_refresh_config(
    cap_file: &CapabilitiesFile,
) -> Option<crate::tools::wasm::OAuthRefreshConfig> {
    let auth = cap_file.auth.as_ref()?;
    let oauth = auth.oauth.as_ref()?;
    let builtin =
        crate::cli::oauth_defaults::builtin_credentials(&canonical_secret_name(&auth.secret_name));
    ...
    Some(crate::tools::wasm::OAuthRefreshConfig {
