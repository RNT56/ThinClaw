...
use thinclaw::{
    ...
    app::{
        AppBuilder, AppBuilderFlags, LocalRuntimeChannel, PeriodicPersistencePlan,
        QuietStartupSpinner, RuntimeEntrypointPlan, RuntimeEnvBootstrapPlan, init_cli_tracing,
        should_show_quiet_startup_spinner,
    },
    channels::{GatewayChannel, TuiChannel, web::log_layer::LogBroadcaster},
    cli::{
...
pub(crate) async fn async_main() -> anyhow::Result<()> {
    ...
    let runtime_entry_mode = command_intent.initial_entry_mode();
    ...
    let Some(mut runtime_entry_mode) =
        command_dispatch::prepare_runtime_entry(&cli, runtime_entry_mode).await?
    else {
        return Ok(());
    };
    ...
    let channel_setup::ChannelSetup {
        channels,
        channel_plan,
        mut channel_names,
