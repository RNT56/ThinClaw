# Scrappy: Development Setup Guide

> **Last updated:** 2026-02-23

Scrappy is a cross-platform desktop AI application built with Tauri (Rust + React). It supports **multiple inference engines** — the engine is selected at compile time via Cargo feature flags.
...
2. Then `setup:all` runs four sub-scripts in sequence:
...
Scrappy provides **dedicated npm scripts** for building each engine variant. Each script:
...
```
**Bundles:** llama-server, whisper-server, sd, piper, node, Chromium, all `.dylib`/`.metal` assets, OpenClaw engine.
...
```
**Bundles:** `uv` (Python manager), whisper-server, piper, node, Chromium, OpenClaw engine.  
...
```
**Bundles:** `uv`, whisper-server, piper, node, Chromium, OpenClaw engine.  
...
```
**Bundles:** whisper-server, piper, node, Chromium, OpenClaw engine.  
...
```
**Bundles:** node, Chromium, OpenClaw engine.  
...
- Install Ollama from [ollama.ai](https://ollama.ai) and run `ollama serve` before starting Scrappy.
...
### Unified Memory
Scrappy takes advantage of macOS Unified Memory. In **Settings → Server & Memory**, allocate sufficient RAM for your models. Metal handles the distribution between CPU/GPU automatically.
...
- **Saving a key does NOT automatically grant it** to the OpenClaw agent. After saving, you must toggle **"Grant Access"** for each key in Settings › Secrets. This ensures keys are only exposed to the agent when you explicitly permit it.
