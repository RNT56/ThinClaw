...
**Current state (compiles-unused).** Locations: `src/qr_pairing.rs:1-329`; `src/lib.rs:83` (`pub mod qr_pairing;`); security defects at `:219` (non-constant-time token compare `if self.info.pairing_token == token`), `:94` (hand-rolled base64), `:88` (`generate_pairing_token` via per-byte `rand::random`). Live alternative `src/pairing/store.rs:1-?`, `:199` (`PairingStore::new`).

**Is there an alternative?** Yes — `src/pairing/store.rs` (`PairingStore`), the live persistent device/channel pairing mechanism, wired at `src/setup/channels.rs`, `src/cli/pairing.rs`, `src/channels/wasm/mod.rs`, and `tests/wasm_channel_integration.rs`. Provides pending-request TTL, max-pending caps, approve rate-limiting, allow/block lists, and on-disk persistence — none of which `qr_pairing.rs` offers.
