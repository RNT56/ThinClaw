...
> **✅ APPLIED 2026-06-25 (Wave 4 deletion batch, commit `4f26f5f4`).** All entries below (D-01–D-06 + the WS-11 erase list: `src/safety/*` orphans, dead CLI stubs, `self_message`, `qr_pairing`, `tailscale`, misc helpers + HTTPS cred mappings, `RepairTask`, heartbeat runner, `SmartRoutingProvider`, `InferenceRouter` chat, leaky `safety`/`executor` fields) were ERASED after operator sign-off and a per-item evidence review ([DELETION-DOSSIER.md](./DELETION-DOSSIER.md)). ~7K+ lines removed; `cargo check` + `clippy --all-targets -D warnings` + tests all green. `voice_wake` was the one item KEPT and **WIRED** (not deleted; operator chose "build the wake word"): it is now live, constructed in `AppBuilder` (`src/app.rs`) and its events consumed in `src/async_main.rs` (voice-wake dispatch, commit `28f76f7f`), behind the opt-in `voice` feature. The entries are retained below as the historical record. Verify current deletions against the tree; `src/tailscale.rs`, `src/qr_pairing.rs`, `src/cli/{nodes,subagent_spawn,session_export}.rs`, and the `src/safety/*` orphans no longer exist.**
