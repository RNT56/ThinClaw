...
- **wasmtime advisory — FIXED.** Was: `wasmtime`/`wasmtime-wasi` pinned `36.0.10` (RUSTSEC-2026-0182), red on the default `wasm-runtime` path. Now: both pinned `36.0.12` in `Cargo.lock` (`Cargo.toml` still requests `version = "36"`), and `cargo deny check advisories` no longer flags the advisory.
