...
| 6 | Channel manager and native channels | `src/channels/manager.rs`, `src/channels/channel.rs`, `src/channels/{http,signal,gmail,apple_mail,imessage,nostr,discord}.rs`, `src/channels/{gmail_wiring,imessage_wiring,health_monitor,status_view}.rs`, `channels-docs/*` | inbound event -> identity/session resolution -> agent turn -> outbound formatting/delivery |
...
| 9 | Routines, heartbeat, subagents, self-repair, and agent management | `src/agent/{routine,routine_engine,scheduler,heartbeat,subagent_executor,agent_registry,self_repair,worker,job_monitor,undo,compaction}.rs` | scheduled/background work -> delegated work -> recovery/progress -> audit trail |
