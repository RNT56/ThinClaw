...
Most of the gateway's mobile-relevant API surface is described by an OpenAPI
document generated **from the Rust source** (utoipa annotations behind the
...
but are **not** annotated with `#[utoipa::path]`, so they do not appear in the
committed snapshot and are not covered by the drift gate:
`GET /api/jobs/{id}/events` (the polled job event log),
`GET /api/jobs/{id}/files/list`, and `GET /api/jobs/{id}/files/read`. The Swift
client decodes these by hand — `ThinClawCore.JobEvent` is explicitly a
hand-rolled decode target, not a generated type. Treat the snapshot as the
contract for the generated `ThinClawAPI` client, not as the complete inventory
of device-scoped routes.
...
- Five targets (app, widgets, notification-service extension, watch app, watch
  widgets); generated `.xcodeproj` is never committed.
...
`ThinClawAPI` REST client. All six of these M1 layers pass `swift test` on macOS
