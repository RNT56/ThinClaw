...
This guide covers configuring the Telegram channel for ThinClaw, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with ThinClaw via Telegram DMs and groups. It supports:
...
Edit `~/.thinclaw/channels/telegram.capabilities.json` (or the config injected by the host):
...
# Install
mkdir -p ~/.thinclaw/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.thinclaw/channels/
```
...
- **Secrets store**: `thinclaw` CLI (if available)
...
- Set `bot_username` in config to your bot's username (e.g., `MyThinClawBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `thinclaw run`
