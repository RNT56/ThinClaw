...
| Source build on macOS | Xcode Command Line Tools, Rust 1.94+, `wasm32-wasip2` target, Git | `wasm-tools`/`cargo-component` for building WASM extensions; Docker for sandbox jobs; optional feature dependencies from [EXTERNAL_DEPENDENCIES.md](EXTERNAL_DEPENDENCIES.md) |
| Source build on Linux | C/C++ build tools, `pkg-config`, `curl`, Git, Rust 1.94+ | `wasm32-wasip2` for WASM extension builds; `libasound2-dev` when enabling `voice`; Docker, browser, and desktop packages as needed |
