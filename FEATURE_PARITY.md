...
| Hook reliability controls | ❌ | ✅ | P2 | Per-hook consecutive-failure tracking auto-disables a hook after 5 failures in a row (`MAX_CONSECUTIVE_HOOK_FAILURES`), with manual re-enable; hook contexts now deliver real invocation metadata instead of an always-empty default; hook payloads are validated against their declared `HookPoint` shape before dispatch. An additive, typed `HookPatch` mechanism exists for hooks to return structured partial mutations, but it is not yet consumed by any dispatcher/agent-loop call site — only `HookPatch::LlmInput` is honored when applied manually today ([`src/hooks/registry.rs`](src/hooks/registry.rs), [`src/hooks/hook.rs`](src/hooks/hook.rs), [`crates/thinclaw-agent/src/hook_dispatch.rs`](crates/thinclaw-agent/src/hook_dispatch.rs)) |
