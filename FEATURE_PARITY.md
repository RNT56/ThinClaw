...
| Unified Desktop secrets | implemented on macOS — one keychain-backed `SecretStore` serves Workbench host reads and the Cockpit's grant-aware runtime trait; its single persisted JSON map is a core AES-256-GCM/HKDF authenticated envelope with a separate Keychain master-key item. Startup rejects corrupt/unavailable vaults, plaintext and per-key legacy sources migrate transactionally, clones share live grants, aliases migrate canonically, deletion revokes access, and custom-secret Update is functional. Windows/Linux still require a durable OS-secret backend before packaging can claim credential persistence. | `apps/desktop/backend/src/thinclaw/config/keychain.rs`, `apps/desktop/backend/src/secret_store.rs`, `apps/desktop/backend/src/thinclaw/secrets_adapter.rs`, `apps/desktop/frontend/src/components/settings/SecretsTab.tsx` |
