...
> **Last reconciled:** 2026-07-14 (Desktop distribution: product/Desktop versions align; release CI fails closed on missing Apple/updater credentials, packages the pinned/budgeted llama.cpp + Chromium runtime, verifies signed/notarized/stapled Apple Silicon artifacts, and emits the tested static updater contract. `UpdateChecker` acceptance covers check/install/relaunch/failure, and agent-eval acceptance now drives isolated `AgentLoopEnv` runtime sessions with provider token capture). Prior: 2026-07-13 (Desktop parity: context-window pressure now crosses the channel, gateway SSE, local/remote Desktop bridge, and typed `UiEvent` boundaries; active-session warning/critical badges clear on recovery, session changes, and disconnects. Scheduled `SystemEvent` heartbeat messages are creatable through the tested routine contract; Trajectory exposes the canonical bounded SFT/DPO exporter; Learning Review shows bounded profile-evolution state and can explicitly upsert/run the reserved weekly routine). Prior: 2026-07-13 (Desktop shared-services work: generated command/event bridge normalization plus unified secrets, model/provider registry, local conversation history, settings, and theming. In the default local profile, Direct and embedded-agent conversations share one surface-isolated runtime database and `ConfigManager` provides versioned Workbench and Agent views over its settings table. In the PostgreSQL profile, Direct Workbench retains its canonical local history and recovery settings while the embedded agent uses the configured remote database. One versioned UI-theme record and semantic token application path serve both product surfaces in either profile). Prior: 2026-07-10 (documentation audit: reconciled the matrix against the Wave-4 dead-code erasures. Corrected the removed Tailscale passwordless-identity auth, `DangerousToolTracker`, `self_message`/`TrustedMetadata`, and `/subagents` slash-command rows to reflect that those capabilities no longer exist, renamed the `nodes` CLI row to `devices`, and repointed decomposed source anchors to their new module paths). Prior: 2026-07-05 (audit-driven hardening: content-aware/CJK token estimation with tool-call accounting, cross-turn tool-call/result continuity in reconstructed context, provider context-overflow error classification into `LlmError::ContextLengthExceeded`, sub-agent cost-guard enforcement + per-principal concurrency cap, background MCP health monitor populating `McpRuntimeHealth` with auto-reconnect, Gmail unattended OAuth-refresh loop, zero-config `web_search` built-in tool, removal of superseded dead action tools). Prior: 2026-07-02 agent-loop hardening (dispatcher transient retry, credential-aware failover, compaction summary re-injection, model-derived context window, routine failure backoff/auto-disable, two-phase subagent cancellation; runtime-hardening batch: per-conversation dispatch concurrency, renewable routine run-leases, durable `/undo`/`/clear` watermark, persisted `/personality` overlay, token-based context caps, declarative command registry, structured worker completion, durable subagent ledger, verifier-based eval rewards, hydrated trajectory export, per-hook failure auto-disable, severity-driven prompt sanitization, pooled/cached learning-provider orchestrator)
...
| Tailscale integration | ✅ | ✅ | Full tunnel module (`tunnel/tailscale.rs`) with Serve + Funnel. Desktop Local Core mounts an authenticated loopback gateway and adds generated lifecycle/status commands plus a private-by-default Remote Access panel; public Funnel requires explicit per-start confirmation. |
...
| `/personality` | ❌ | ✅ | - | Personality overlay command (`/personality`, `/personality <name>`, `/personality reset`) with `/vibe` retained as a compatibility alias. The overlay now persists across restart via a dedicated `personality_overlay` conversation-metadata key restored on hydration, mirroring `/model`'s hydration-restore pattern. Desktop Agent Chat exposes that canonical path through an accessible status/preset/restore control without introducing a competing state API ([`src/agent/commands.rs`](src/agent/commands.rs), [`src/agent/thread_ops/hydration.rs`](src/agent/thread_ops/hydration.rs), [`apps/desktop/frontend/src/components/thinclaw/chat/PersonalityControl.tsx`](apps/desktop/frontend/src/components/thinclaw/chat/PersonalityControl.tsx)) |
...
| Context compaction | ✅ | ✅ | Auto summarization with user-visible pressure warnings at 85%/95% before compaction. Structured `ContextPressure` transitions also drive the active Desktop session's accessible amber/red capacity badge in both local and remote modes and explicitly clear it after recovery. The generated summary is re-injected into the post-compaction context fragment (and persisted with it) so the model keeps the gist of dropped turns, and the compaction trigger derives its context window from the active model's catalog entry instead of a fixed 100k default ([`src/agent/thread_ops/input.rs`](src/agent/thread_ops/input.rs), [`src/agent/thread_ops/compaction_context.rs`](src/agent/thread_ops/compaction_context.rs), [`apps/desktop/frontend/src/components/thinclaw/ContextPressureBadge.tsx`](apps/desktop/frontend/src/components/thinclaw/ContextPressureBadge.tsx)) |
...
| Optional external memory providers | ❌ | ✅ | Honcho, Zep, Mem0, OpenMemory, Letta, Chroma, Qdrant, and custom HTTP adapters with local-first canonical memory and non-fatal provider fallback. Providers share a `LearningOrchestrator`-owned manager with a 60-second readiness/health cache and one pooled HTTP client. Desktop Learning Review adds generated setup/disable commands, health/config UI, and a secret-safe contract that accepts environment-variable references rather than raw key values ([`src/agent/learning/providers/mod.rs`](src/agent/learning/providers/mod.rs), [`src/agent/learning/orchestrator/`](src/agent/learning/orchestrator/), [`apps/desktop/frontend/src/components/thinclaw/learning/ExternalMemoryPanel.tsx`](apps/desktop/frontend/src/components/thinclaw/learning/ExternalMemoryPanel.tsx)) |
...
| Trajectory archive + training export | ❌ | ✅ | Per-turn trajectory logging stores structured assessment metadata and exports `json`, `jsonl`, `sft`, and `dpo` datasets with validation, score filtering, max-record limits, skipped-record counts, and optional manifests for offline training workflows. CLI and Desktop share one exporter over legacy plus durable run-artifact records; Desktop exposes SFT/DPO through a generated local-only command, a 5,000-example IPC cap, and explicit download buttons. `trajectory export`/`trajectory stats` best-effort hydrate records with stored feedback/evaluations, falling back to status-derived labeling when the DB is unavailable. Subagent learning confidence is derived from observable signals rather than hardcoded success/failure constants ([`src/agent/learning/trajectory.rs`](src/agent/learning/trajectory.rs), [`src/cli/trajectory.rs`](src/cli/trajectory.rs), [`apps/desktop/backend/src/thinclaw/commands/rpc_trajectory.rs`](apps/desktop/backend/src/thinclaw/commands/rpc_trajectory.rs), [`crates/thinclaw-agent/src/subagent.rs`](crates/thinclaw-agent/src/subagent.rs)) |
| Verifier-based eval harness | ❌ | ✅ | Benchmark environments execute and score the agent's own action (e.g. running its shell command, checking its own answer) rather than comparing against a canned response, with a graded heuristic fallback (0.0 empty/error, 0.3 trivial, 0.55 substantive) kept below the `SFT_QUALITY_GATE_SCORE` (0.6) gate when no verifier applies. Episodes run concurrently via `buffer_unordered` with a concurrency of 4. Desktop exposes the local `agent_loop` environment through a Benchmarks panel with bounded episode/step controls and scored summaries; deterministic acceptance drives isolated restricted agent sessions and provider token-capture propagation, while verifier case suites remain CLI-driven ([`crates/thinclaw-agent/src/env.rs`](crates/thinclaw-agent/src/env.rs), [`apps/desktop/frontend/src/components/thinclaw/experiments/BenchmarkPanel.tsx`](apps/desktop/frontend/src/components/thinclaw/experiments/BenchmarkPanel.tsx)) |
...
> ⚠️ **Experimental — first signed release qualification pending.** Desktop parity
> and committed packaging automation are complete; production readiness still
> requires provisioning Apple/updater secrets and validating the first notarized
> DMG on a clean Mac.
...
|---|---|---|
| Dual-mode bridge contract: `RouteMode`, typed `BridgeError`, `gated()`, `ROUTE_TABLE` linter | ✅ — `ROUTE_TABLE` classifies all registered commands and generates the drift-guarded route matrix; every command now returns the tagged error taxonomy and the UI preserves kind/remediation/retryability through a shared normalizer. Generated bindings are tested to contain zero raw string error channels. | `thinclaw/bridge.rs`, `command-errors.ts`, `remote-gateway-route-matrix.md` |
| Desktop remote-gateway resilience | ✅ — idempotent reads use bounded transient retries and capped `Retry-After`; mutations remain at-most-once by default. HTTP/transport failures retain typed retryability, SSE reconnect is bounded and shutdown-safe, non-transient stream failures stop, and the UI deduplicates reconnect/recovered notices. | `remote_proxy/core.rs`, `remote_proxy/sse.rs`, `ThinClawChatView.tsx` |
| Desktop performance budgets | ✅ — backend/renderer readiness is measured against explicit budgets; adjacent token deltas coalesce within one 16 ms frame while other events preserve immediate ordering; Direct and Agent histories are both virtualized; chunks remain capped at 500 KiB; runtime metrics split Desktop and recursive sidecar memory and flag the configured ceiling. | `performance-budgets.md`, `thinclaw-event-buffer.ts`, `ThinClawChatView.tsx`, `system.rs` |
| Current Claude catalog and defaults | ✅ — official Claude Platform metadata (verified 2026-07-13) drives Fable 5, Opus 4.8, Sonnet 5, and Haiku 4.5 IDs, context/output windows, standard pricing, setup fallbacks, Desktop onboarding, and Bedrock defaults. Opus 4.8 is the default for complex agentic work; Sonnet 5 fills the balanced routing slot. | `registry/models.json`, `registry/providers.json`, `provider_catalog.rs`, `setup/wizard/helpers.rs`, `LLM_PROVIDERS.md` |
| Reproducible local-engine matrix | ✅ — llama.cpp b9988 and uv 0.11.28 assets are SHA-256/version verified; current MLX/vLLM direct dependencies use versioned bootstrap fingerprints; Ollama exposes its external daemon version; GGUF metadata is bounded and current quant types are validated before sidecar spawn. | `engine-compatibility.md`, `engine/mod.rs`, `gguf.rs`, `setup_llama.sh`, `setup_uv.sh` |
| Desktop release and updater channel | implemented, external qualification pending — product/Desktop versions align; tag CI requires production credentials, imports an ephemeral Developer ID identity, builds and verifies a signed/notarized/stapled Apple Silicon DMG, publishes a signed `darwin-aarch64` updater manifest, and blocks cargo-dist publication on failure. Core llama.cpp/Chromium archives are checksum verified before replacement and size-budgeted through clean-machine fixtures. | `.github/workflows/release.yml`, `apps/desktop/scripts/`, `packaging-platform-readiness.md`, `UpdateChecker.tsx` |
| Desktop security and dependency baseline | ✅ — main and Spotlight windows have separate least-privilege capabilities; shell execution, arbitrary file opening, and home-wide filesystem scopes are absent; file reveal is canonicalized against app-data/workspace roots. npm is audit-clean, Tauri/ORT/OpenDAL are refreshed, patched XML/QUIC releases are resolved, and enabled-graph Rust advisories pass with no ignore list. | `security-and-dependencies.md`, `capabilities/*.json`, `validate_packaging_readiness.sh`, `deny.toml` |
| Generated frontend command client | implemented — 224 compatibility calls and 43 direct consumers delegate through generated `commands.*`; the stable `lib/thinclaw.ts` barrel routes consumers to five focused `lib/api` domains; raw `invoke`/string commands are contract-test forbidden across production frontend source | `lib/command-client.ts`, `lib/thinclaw.ts`, `lib/api/*`, `setup/commands.rs` |
| Typed desktop event bus | implemented — generated `UiEvent` union, one native `thinclaw-event` listener, 11 typed subscriptions; local and remote channel-status changes share the same event shape | `ui_types.rs`, `event_mapping.rs`, `use-thinclaw-stream.ts` |
| Backend Desktop service boundary | implemented — reusable helpers moved from the retired root Tauri facade to `desktop_api`; typed command modules are the only registration/wire adapters, with a deprecated compatibility alias | `src/desktop_api.rs`, `src/lib.rs`, `thinclaw/commands/*` |
| Shared Desktop services seam | implemented — one typed `SharedServices` Tauri state delegates to existing host singletons and one injectable React services context wraps both Desktop modes; no duplicate cache, pool, registry, or runtime is created | `apps/desktop/backend/src/shared_services.rs`, `apps/desktop/frontend/src/components/services-context.tsx`, `apps/desktop/frontend/src/App.tsx` |
| Unified Desktop secrets | implemented on macOS — one keychain-backed `SecretStore` serves Workbench host reads and the Cockpit's grant-aware runtime trait; its single persisted JSON map is a core AES-256-GCM/HKDF authenticated envelope with a separate Keychain master-key item. Startup rejects corrupt/unavailable vaults, plaintext and per-key legacy sources migrate transactionally, clones share live grants, aliases migrate canonically, deletion revokes access, and custom-secret Update is functional. Secrets Settings also supports checksummed recovery-key export/import and verified transactional master-key rotation with envelope/key rollback. Windows/Linux still require a durable OS-secret backend before packaging can claim credential persistence or recovery. | `apps/desktop/backend/src/thinclaw/config/keychain.rs`, `apps/desktop/backend/src/thinclaw/commands/rpc_secret_recovery.rs`, `apps/desktop/backend/src/secret_store.rs`, `apps/desktop/backend/src/thinclaw/secrets_adapter.rs`, `apps/desktop/frontend/src/components/settings/{SecretsTab.tsx,storage/RecoveryKeyPanel.tsx}` |
| Unified model/provider registry | implemented — one cloneable `ModelProviderRegistry` owns provider metadata, cloud discovery cache, local inventory, local-runtime snapshots, and provider-key readiness; the inference router shares that service instead of maintaining a second key-vault/metadata path | `apps/desktop/backend/src/inference/model_discovery/mod.rs`, `apps/desktop/backend/src/inference/router.rs`, `apps/desktop/backend/src/model_manager.rs` |
| Unified Desktop conversation history | implemented — in the default local profile, `SharedHistoryStore` opens one canonical runtime database, injects the same store into the embedded agent, isolates `direct_workbench` from `agent_cockpit` rows by `surface`, and deterministically merges legacy Direct SQLite conversations/messages with metadata and attachments intact. The PostgreSQL profile keeps Direct Workbench history in that local database while the embedded agent uses PostgreSQL | `apps/desktop/backend/src/history.rs`, `apps/desktop/backend/src/thinclaw/runtime_builder.rs`, `migrations/V30__conversation_surfaces.sql` |
| Unified Desktop settings | implemented — in the default local profile, one versioned `ConfigManager` persists the typed Workbench view and Agent key/value view in the canonical runtime settings table; legacy `user_config.json` merges once and remains a recovery mirror; the System Control panel switches and saves the explicit two-view envelope instead of writing bogus `raw`/`baseHash` keys. The PostgreSQL profile retains the recovery-backed Workbench settings path while the embedded agent uses its remote settings store | `apps/desktop/backend/src/config.rs`, `apps/desktop/backend/src/thinclaw/commands/rpc_config.rs`, `apps/desktop/frontend/src/components/thinclaw/ThinClawSystemControl.tsx` |
| Unified Desktop theming | implemented — one v2 preference record with tested v1/legacy migration, compact/comfortable density, live multi-window sync, and one semantic token application path feed both Direct Workbench and Agent Cockpit; legacy Cockpit zinc/cyan/indigo utilities inherit the selected app palette while status colors retain their meaning | `apps/desktop/frontend/src/components/theme-provider.tsx`, `apps/desktop/frontend/src/index.css`, `apps/desktop/frontend/src/components/chat/views/ThinClawView.tsx` |
| Desktop design system | implemented — stable spacing, radius, control-height, motion, and density-ready CSS tokens sit above every color palette; typed Button, Surface, Progress, and AsyncState primitives provide consistent interaction and assistive semantics across Workbench and Cockpit | `apps/desktop/frontend/src/lib/app-themes.ts`, `apps/desktop/frontend/src/components/ui/`, `apps/desktop/frontend/src/index.css` |
| Desktop product-mode seam | implemented — the sidebar always exposes labeled Workbench, Agent Cockpit, and Imagine destinations (including when collapsed), identifies live agent-runtime status, supports roving arrow-key navigation and direct Mod+1/2/3 switching, and shares a searchable, focus-trapped Mod+K command palette for modes and settings | `apps/desktop/frontend/src/components/navigation/{ModeNavigator.tsx,CommandPalette.tsx}`, `apps/desktop/frontend/src/components/chat/{ChatLayout.tsx,ChatProvider.tsx}` |
| Unified Desktop onboarding | implemented — one tested, adaptive first-run route serves local and remote deployments and configures shared appearance, agent identity/personality, engine setup, local/cloud inference, model or provider setup, OS permissions, and the authoritative post-setup channel entrypoint; identity writes use the same canonical Agent settings command in either deployment mode | `apps/desktop/frontend/src/components/onboarding/OnboardingWizard.tsx`, `apps/desktop/documentation/setup.md` |
| Desktop accessibility baseline | implemented — all product modes remain keyboard reachable in the collapsed shell; roving mode navigation, direct shortcuts, a focus-trapped command dialog, focus-safe sidebar expansion, named/current settings controls, dialog/progress/live-state semantics, shared visible focus, reduced-motion handling, and forced-colors support are covered by component regressions | `apps/desktop/documentation/accessibility.md`, `apps/desktop/frontend/src/index.css`, `apps/desktop/frontend/src/components/{navigation,ui}/` |
| Desktop localization bridge | implemented — the Rust catalog is the authoritative source for a generated local-only Tauri command; React normalizes browser locale variants, merges complete English fallback, persists and synchronizes language across windows, updates document language, and translates the shared shell, command palette, loading states, and Appearance selector | `src/i18n.rs`, `apps/desktop/backend/src/i18n.rs`, `apps/desktop/frontend/src/components/i18n-provider.tsx` |
| Desktop interaction-state polish | implemented — typed loading/empty/error/progress primitives now drive app/view startup, Direct conversation loading and first-message guidance, Agent control-surface loading, context-window progress, and onboarding progress; density changes are immediate and cross-window through the migrated theme record, while motion respects the shared duration and reduced-motion contracts | `apps/desktop/frontend/src/components/ui/`, `apps/desktop/frontend/src/components/chat/views/`, `apps/desktop/frontend/src/components/theme-provider.tsx` |
| Embedded Agent runtime assembly | implemented — the stable builder entry point coordinates focused environment/provider, background-task, event-forwarder, and optional sandbox modules; runtime state and shutdown ownership remain unchanged | `apps/desktop/backend/src/thinclaw/runtime_builder.rs`, `apps/desktop/backend/src/thinclaw/runtime_builder/*` |
| Repo Projects Cockpit composition | implemented — the public panel delegates live/shell state and refresh behavior to one hook, with typed fixtures, readiness/event utilities, and shared sub-panels isolated for testing; command behavior is unchanged | `apps/desktop/frontend/src/components/thinclaw/ThinClawRepoProjects.tsx`, `apps/desktop/frontend/src/components/thinclaw/repo-projects/*` |
| Lifecycle Hooks Cockpit composition | implemented — hook loading and mutations live in one data hook; active-hook cards, template cards, the custom editor, and the characterized built-in catalog are independent modules behind the unchanged public panel | `apps/desktop/frontend/src/components/thinclaw/ThinClawHooks.tsx`, `apps/desktop/frontend/src/components/thinclaw/hooks/*` |
| Automations Cockpit composition | implemented — routine polling, lifecycle events, CRUD, history, and lint state live in one hook; job cards, schedule presets, and the creation flow for both agent jobs and heartbeat system events are independent modules behind the unchanged public panel | `apps/desktop/frontend/src/components/thinclaw/ThinClawAutomations.tsx`, `apps/desktop/frontend/src/components/thinclaw/automations/*` |
| Sub-agent, Channels, and Skills composition | implemented — all three public panels remain stable and below 500 lines; child-session rendering, channel/stream metadata and state, skill cards, normalization, and mutations live in focused tested modules | `apps/desktop/frontend/src/components/thinclaw/{SubAgentPanel,ThinClawChannels,ThinClawSkills}.tsx`, `apps/desktop/frontend/src/components/thinclaw/{sub-agents,channels,skills}/*` |
| Desktop dual-mode fixture acceptance | implemented — one credential-free CI harness proves representative embedded-mode availability and typed gates, then exercises authenticated loopback gateway chat/session plus management routes (jobs, autonomy, learning, experiments, MCP, skills, providers, costs, and cache) with exact request/header assertions | `scripts/ci/desktop-fixture-acceptance.sh`, `apps/desktop/backend/src/thinclaw/{bridge.rs,remote_proxy/tests.rs}` |
| Desktop command-contract drift guard | implemented — tests regenerate the complete Specta command registry in memory and require a byte-identical sanitized binding, preserve real `Channel<T>` transport types, reject duplicate command names and reserved TypeScript parameters, and prove sanitizer idempotence | `apps/desktop/backend/src/setup/commands.rs`, `apps/desktop/backend/src/lib.rs`, `apps/desktop/frontend/src/lib/bindings.ts` |
| Desktop top-journey E2E | implemented — browser-mode WebdriverIO runs ten user journeys across onboarding, theme selection, the Chat shell, Dashboard, Channels, Automations, Jobs, Models, Secrets, and Appearance, plus a deterministic Tauri IPC contract check; full-shell startup fixtures cover expected native calls without requiring a packaged binary | `apps/desktop/e2e/{onboarding,main-flows}.e2e.ts`, `apps/desktop/wdio.browser.conf.ts` |
| Desktop observer + crash reports | implemented — the operator-selected core observer remains authoritative while an always-on Desktop adapter emits redacted metadata-only records through the existing typed event bus; observer errors and process panics persist locally as private, bounded reports (20-file cap, no upload) | `apps/desktop/backend/src/thinclaw/desktop_observer.rs`, `apps/desktop/frontend/src/components/thinclaw/ThinClawEventInspector.tsx`, `docs/OBSERVABILITY.md` |
| Undo / redo | implemented — generated local/remote commands route canonical `/undo` and `/redo` behavior, with accessible cockpit toolbar actions and tests | `commands/sessions.rs`, `ThinClawChatView.tsx` |
...
| Trajectory viewer and training export | implemented — stats/records plus bounded canonical SFT/DPO export, generated binding, explicit download controls, and UI/format contract tests | `rpc_trajectory.rs`, `ThinClawTrajectory.tsx`, `src/cli/trajectory.rs` |
| Profile evolution control | implemented — Learning Review shows parsed profile confidence/evidence plus reserved-routine schedule/run/failure state; explicit execution upserts the actor-scoped routine from first-party profile material and starts it through the live engine | `rpc_profile_evolution.rs`, `learning/ProfileEvolutionPanel.tsx`, `src/profile_evolution.rs` |
| External memory control | implemented — Learning Review configures and activates all eight registered providers, shows live health/readiness, and deactivates the active provider; Desktop persists only validated API-key environment-variable names and returns a typed local-only gate for remote mutation | `rpc_experiments_learning.rs`, `learning/ExternalMemoryPanel.tsx`, `src/agent/learning/providers/*` |
| Desktop remote access | implemented — Local Core mounts the root authenticated gateway on loopback with the same database, sessions, tools, channel manager, and credential shown by Gateway settings. Generated local-only status/start/stop commands own Tailscale Serve/Funnel; the panel defaults to private tailnet exposure, requires explicit confirmation for every public start, accepts no Tailscale auth keys, and resets exposure during app shutdown | `runtime_builder.rs`, `remote_access.rs`, `ThinClawRemoteAccess.tsx`, `src/tunnel/tailscale.rs` |
| Fleet command | implemented — local and saved remote agents form one explicit fleet model; remote health comes from authenticated gateway status, selected-agent task assignment is delivered to that gateway, and broadcast produces one delivery receipt per node instead of duplicating prompts across local threads | `thinclaw/fleet.rs`, `rpc_orchestration.rs`, `fleet/FleetCommandCenter.tsx` |
| Agent eval | implemented — generated list/run commands feed the interactive Benchmarks panel; bounded evaluation uses the embedded agent and executable acceptance covers restricted isolated sessions and provider capture | `rpc_experiments_learning.rs`, `BenchmarkPanel.tsx`, `crates/thinclaw-agent/src/env.rs` |
...
| Channel runtime config | implemented — schema-driven local/remote forms validate typed settings, route WASM manifest credentials only to encrypted secret storage, preserve current native values, and show honest host-managed states where runtime identity cannot be changed in-process | `Channel::config_schema()`, WASM capabilities mapper, `rpc_channel_config.rs`, `ThinClawChannelConfig.tsx` |
| Honest sidecar status | implemented — image/TTS rows report configured capability separately from live process state and preserve explicit unavailable reasons | `sidecar/{commands,types,core}.rs`, `hooks/use-chat.ts` |
| Tool-policy enforcement | implemented — dispatcher preflight and post-hook argument policy enforce deny/approval outcomes; the read-only Security panel exposes current effective policy metadata without treating telemetry as enforcement | `tool_execution.rs`, `rpc_security.rs`, `SecurityPosturePanel.tsx` |
...
| Config editing | ✅ | ✅ | P3 | `Settings.set()/.get()/.list()/.reset()` with typed path-based access ([`crates/thinclaw-settings/src/settings_impl.rs`](crates/thinclaw-settings/src/settings_impl.rs)). Web gateway Settings tab with grouped sections (Notifications, Heartbeat, Agent, Channels [Telegram/Signal/Discord/Slack/Nostr/iMessage/Gmail/Gateway], Safety, Features), toggle switches, import/export. Desktop's generated channel-config surface validates typed current values for Signal, Discord, iMessage, Nostr, Apple Mail, and BlueBubbles; maps installed WASM manifest setup fields without returning credentials; and gives explicit host-managed instructions for native lifecycle transports. Credential replacements go only to encrypted secret storage. Includes `agent.main_tool_profile`, `agent.worker_tool_profile`, `agent.subagent_tool_profile`, `agent.subagent_transparency_level`, and `channels.telegram_subagent_session_mode` plumbing |
...
| Control UI i18n | ✅ | ✅ | - | EN/ES/ZH/JA/KO/DE/FR/PT/RU catalogs with normalized locale lookup and complete English fallback; Desktop consumes the core catalog through a generated Tauri contract and synchronizes language across windows |
...
| Scheduled heartbeat system events | ❌ | ✅ | - | Desktop and gateway creation accept `Trigger::SystemEvent`, validate its schedule and non-empty message, and queue the message for the next heartbeat tick. Existing clients remain cron-by-default. |
...
| Weekly profile evolution | ❌ | ✅ | - | A reserved actor-scoped routine conservatively refreshes the psychographic profile from first-party conversation evidence. Desktop Learning Review exposes bounded parsed state, scheduling/run diagnostics, and explicit manual execution without bypassing the routine engine's concurrency guard. |
...
| Sandbox env sanitization | ✅ | ✅ | Shell tool scrubs env vars + LD*/DYLD* injection blocks + executable allowlist |
...
| Shell sandbox (3-layer) | ❌ | ✅ | When `base_dir` set: (1) executable allowlist auto-enforced, with network/mutating executables routed through approval, (2) workdir validation (must be under base_dir), (3) command path scanning (`detect_path_escape` + `..` traversal detection); host temp paths are default-denied and explicitly configurable. The first-party pre-exec scanner fails closed by default and is resolved from configured path / `PATH` / bundled asset / verified cache ([`src/tools/builtin/shell.rs`](src/tools/builtin/shell.rs), [`src/tools/builtin/shell_security.rs`](src/tools/builtin/shell_security.rs), [`src/bin/thinclaw-shell-scan.rs`](src/bin/thinclaw-shell-scan.rs)) |
...
| Executable allowlist | ✅ | ✅ | `THINCLAW_ALLOWED_EXECUTABLES_ONLY` + extensible `THINCLAW_EXTRA_EXECUTABLES`; legacy `SAFE_BINS` names remain aliases. Read-only and approval-required network/mutation categories are explicit |
...
| Credential theft via env injection | ✅ | ✅ | Shell env scrubbing + command injection detection + LD*/DYLD* blocking + executable allowlist |
...
| Desktop bridge and remote-auth hardening | ❌ | ✅ | Runtime/SSE/Markdown/error content is treated as untrusted and bounded before React; external links and session IDs are validated. The remote proxy refuses redirects and public plaintext bearer transport, proves credentials through authenticated status, caps bodies/SSE, and avoids payload logs. Profile, active gateway, custom-LLM, Gmail OAuth, and deploy credentials are encrypted/redacted with presence-only status and explicit-clear form semantics. SSH deployment validates targets, uses TOFU-with-change-rejection, passes credentials over stdin, discovers Tailscale addressing, and connects on the root gateway port `3000` ([`apps/desktop/documentation/threat-model.md`](apps/desktop/documentation/threat-model.md)). |
...
| `routine_create` | [`routine.rs`](src/tools/builtin/routine.rs) | Create cron, event, webhook, manual, or scheduled system-event routines |
