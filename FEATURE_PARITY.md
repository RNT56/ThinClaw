...
| Routing Engine V2 (unified planner) | ❌ | 🚧 | Unified `RoutePlanner` with structured scoring (quality/cost/latency/health), capability gating, budget-aware cost pressure, session affinity, and context window gating. Supports 4 modes: PrimaryOnly, CheapSplit (preserved), AdvisorExecutor (new), Policy. Shadow mode logs diff vs live path. `consult_advisor` tool for advisor-executor pattern ([`src/llm/route_planner.rs`](src/llm/route_planner.rs), [`src/tools/builtin/advisor.rs`](src/tools/builtin/advisor.rs)) |
