...
| Unified Desktop theming | implemented — one versioned preference record, validated legacy migration, live multi-window sync, and one semantic token application path feed both Direct Workbench and Agent Cockpit; legacy Cockpit zinc/cyan/indigo utilities inherit the selected app palette while status colors retain their meaning | `apps/desktop/frontend/src/components/theme-provider.tsx`, `apps/desktop/frontend/src/index.css`, `apps/desktop/frontend/src/components/chat/views/ThinClawView.tsx` |
