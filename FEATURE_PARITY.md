...
| Household multi-actor identity | ❌ | ✅ | Actor registry + `ResolvedIdentity` + conversation-scope session keys are landed, with `thinclaw access identities ...` management, request-scoped gateway identity resolution, actor-aware protected routes, actor-partitioned SSE/WS fanout, and cross-channel direct-thread continuity that stays bound to the owning actor |
...
| Gateway lock (PID-based) | ✅ | ✅ | `thinclaw runtime web start` writes PID to `~/.thinclaw/gateway.pid`; launchd/systemd keepalive via `thinclaw runtime service` |
...
| Presence system | ✅ | ✅ | Authenticated cross-surface presence over `PUT`/`DELETE /api/presence/{session_id}` + `GET /api/presence`, SSE `presence` (`joined`/`updated`/`expired`) and WebSocket `presence`/`presence_clear` frames with a disconnect lease that cannot erase a newer reconnect. States `online`/`away`/`busy`/`typing` (typing is owned-thread only) across `desktop`/`web`/`ios`/`watchos`/`cli`/`channel`. Server-bounded 5–120s TTL (45s default) with a one-second expiry sweep, caps of 32 sessions per actor / 256 per principal / 4096 registry, and 120 publishes per principal per minute so presence cannot crowd out conversation events. Held only in gateway memory (cleared on restart, no migrations); the public aggregate never contains the principal ID or a session ID. Every mutation is bound to the authenticated principal and actor and reuses the durable actor/thread ownership check; ReadOnly may observe, Operator/Admin may publish, device tokens need the exact `chat`-scope route allowlist, and presence never emits an APNs notification. Because SSE is not replayed, clients fetch the REST snapshot on connect/reconnect then apply ordered transitions. See [`docs/PRESENCE.md`](docs/PRESENCE.md) |
...
| Feature | OpenClaw | ThinClaw | Notes |
|---------|----------|----------|-------|
| Durable sender approval requests | ✅ | ✅ | `thinclaw access senders list/approve` uses non-secret request IDs; host APIs enforce the same model |
...
| `presence` | ✅ | ✅ | P3 | `presence publish <state> [--thread-id]`, `presence list [--thread-id]`, `presence clear <session-id>` — human and machine-readable output over the authenticated REST contract ([`src/cli/presence.rs`](src/cli/presence.rs)) |
...
| Self-update & restart | ❌ | ✅ | `thinclaw runtime update install` + `/restart` command for orderly shutdown; service-managed installs auto-relaunch and foreground `thinclaw` runs now self-relaunch too ([`src/cli/update.rs`](src/cli/update.rs), [`src/agent/submission.rs`](src/agent/submission.rs), [`src/main.rs`](src/main.rs)) |
...
| Workspace storage choices | ✅ | ✅ | ThinClaw exposes only live backends through the shared `WorkspaceStore` contract: libSQL locally/Turso and Postgres when compiled with `postgres`. Former sqlite-vec, LanceDB, and QMD DTO/SQL-only scaffolds were removed because no runtime could select or execute them; a future backend must land with create/upsert/search/delete, restart persistence, dimension validation, and integration coverage before it is exported. |
| Atomic reindexing | ✅ | ✅ | |
...
| Mobile event transport policy | ✅ | ✅ | P2 | iOS explicitly standardizes on one pinned-session SSE policy (`GatewayEventTransportPolicy.mobileDefault`): reconnect/watchdog stays on SSE and live-only gaps reconcile from REST snapshots. The gateway WebSocket remains available to other clients; native WS negotiation/fallback is intentionally outside the mobile contract |
| Presence participation | ✅ | 🚧 | P3 | iOS ties presence renewal and cleanup to the paired foreground `GatewaySession`, waiting for any in-flight renewal to finish before its final clear, and refetches the REST snapshot on every reconnect since SSE is not replayed. The protocol reserves the `watchos` surface so the watch can publish through the same authenticated contract without a revision, but automatic watch lifecycle publication is not wired in this release. Verified by `swift test`; unexercised against a live gateway. See [`docs/PRESENCE.md`](docs/PRESENCE.md) |
...
| Presence indicators | ✅ | ✅ | P3 | Web publishes `away`/`online` on visibility change and a distinct five-second thread-scoped `typing` lease on composer activity, throttled to one renewal per second. Page-local session UUIDs keep duplicated tabs from clearing one another, and each connect/reconnect refetches the authorized REST snapshot before applying live SSE transitions. See [`docs/PRESENCE.md`](docs/PRESENCE.md) |
...
| Tailscale identity (passwordless auth) | ✅ | ✅ | Opt-in explicit user/node/tag→RBAC mappings (`GATEWAY_TAILSCALE_PRINCIPALS`), authenticated direct-peer resolution through bounded local `tailscale whois`, and a loopback-only managed Serve identity-header contract. Funnel is excluded, conflicts fail closed, bearer/device fallback remains, and passwordless streams reauthenticate at most every 60 seconds ([`crates/thinclaw-gateway/src/web/auth.rs`](crates/thinclaw-gateway/src/web/auth.rs), [`src/channels/web/tailscale_identity.rs`](src/channels/web/tailscale_identity.rs)). |
...
| DM sender verification | ✅ | ✅ | `thinclaw access senders approve`, using durable non-secret request IDs; host APIs |
