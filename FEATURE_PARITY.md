...
| Config editing | ✅ | ✅ | P3 | `Settings.set()/.get()/.list()/.reset()` with typed path-based access ([`crates/thinclaw-settings/src/settings_impl.rs`](crates/thinclaw-settings/src/settings_impl.rs)). Web gateway Settings tab with grouped sections (Notifications, Heartbeat, Agent, Channels [Telegram/Signal/Discord/Slack/Nostr/iMessage/Gmail/Gateway], Safety, Features), toggle switches, import/export. Desktop's generated channel-config surface validates schemas and preserves typed current values for Signal, Discord, iMessage, and Nostr; secrets remain on the dedicated encrypted setup path. Includes `agent.main_tool_profile`, `agent.worker_tool_profile`, `agent.subagent_tool_profile`, `agent.subagent_transparency_level`, and `channels.telegram_subagent_session_mode` plumbing |
