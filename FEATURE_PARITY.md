...
| Unified Desktop secrets | implemented — one keychain-backed `SecretStore` serves Workbench host reads and the Cockpit's grant-aware runtime trait; clones share live grants, aliases migrate canonically, deletion revokes access, and custom-secret Update is functional | `apps/desktop/backend/src/secret_store.rs`, `apps/desktop/backend/src/thinclaw/secrets_adapter.rs`, `apps/desktop/frontend/src/components/settings/SecretsTab.tsx` |
