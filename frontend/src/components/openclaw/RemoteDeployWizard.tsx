...
export const RemoteDeployWizard: React.FC<RemoteDeployWizardProps> = ({ isOpen, onCheckStatus, onClose }) => {
    ...
    const startDeploy = async () => {
        ...
        setStep('deploying');
        setLogs(['=== IronClaw Remote Deploy ===', `Target: ${user}@${ip}`]);
        setErrorMsg('');
        ...
        try {
            ...
            await commands.openclawDeployRemote(ip, user, tailscaleKey || null, enableSystemd);
    ...
    const handleConnectAfterDeploy = async () => {
        ...
        try {
            ...
            await commands.openclawSaveGatewaySettings('remote', deployResult.url, deployResult.token || '');
    ...
    const handleDirectConnect = async () => {
        ...
        setTestLoading(true);
        try {
            const ok = await commands.openclawTestConnection(url, existingToken || null);
            if (!ok) {
        ...
        try {
            ...
            await openclaw.addAgentProfile(newProfile);
            await commands.openclawSaveGatewaySettings('remote', url, existingToken || '');
                            ...
                            {deployMode === 'new' ? (
                                ...
                                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 text-sm text-emerald-600 dark:text-emerald-400">
                                        ...
                                        <p className="opacity-90 text-xs font-medium">Connect Scrappy to an already running IronClaw HTTP gateway. A connection test will be performed before saving.</p>
                                    </div>
