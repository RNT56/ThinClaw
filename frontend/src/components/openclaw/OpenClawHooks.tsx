    ...
    },
    {
        ...
        bundle: {
            rules: [{
                ...
                priority: 200,
                append: "\n\n---\n_Powered by IronClaw AI · Responses are generated by AI and may contain errors · Always verify critical information_",
            }],
        },
    },
    {
...
function HookCard({ hook, onRemove }: { hook: openclaw.HookInfoItem; onRemove: () => void }) {
...
export function OpenClawHooks() {
    ...
    const handleActivateTemplate = async (template: HookTemplate) => {
        try {
            const result = await openclaw.registerHookBundle(
    ...
    const handleRemoveHook = async (hookName: string) => {
        try {
            const result = await openclaw.unregisterHook(hookName);
            if (result.ok) {
    ...
    const handleCustomSubmit = async (json: string) => {
        const result = await openclaw.registerHookBundle(json, 'custom');
        if (result.ok) {
