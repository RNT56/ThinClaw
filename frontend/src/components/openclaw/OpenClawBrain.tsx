...
function DbFilesTab() {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await openclaw.writeOpenClawFile(activeFile, content);
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await openclaw.deleteOpenClawFile(activeFile);
...
function buildTree(files: openclaw.WorkspaceFile[]): TreeNode {
    const root: TreeNode = {
        name: '',
        fullPath: '',
        isDirectory: true,
...
function LocalFilesTab() {
    const [files, setFiles] = useState<openclaw.WorkspaceFile[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    ...
    const fetchFiles = async () => {
        ...
        try {
            const [fileList, path] = await Promise.all([
                openclaw.listAgentWorkspaceFiles(),
                openclaw.getWorkspacePath().catch(() => null),
            ]);
            setFiles(fileList ?? []);
    ...
    const handleRevealFile = async (absolutePath: string) => {
        try {
            await openclaw.revealFile(absolutePath);
        } catch (e) {
    ...
    const handleRevealWorkspace = async () => {
        ...
        try {
            await openclaw.revealWorkspace();
        } catch (e) {
