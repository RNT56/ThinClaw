...
import {
    ...
    Cpu, HardDrive, Puzzle, Settings2, Package, Filter
} from 'lucide-react';
...
const SOURCE_CONFIG: Record<string, { icon: typeof Wrench; color: string; label: string }> = {
    ...
    container: { icon: HardDrive, color: 'text-amber-400', label: 'Container' },
    memory: { icon: Settings2, color: 'text-purple-400', label: 'Memory' },
    management: { icon: Package, color: 'text-cyan-400', label: 'Management' },
    extension: { icon: Puzzle, color: 'text-emerald-400', label: 'Extension' },
};
            ...
            <div className="flex-shrink-0 px-5 pt-5 pb-3">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500">{tools.length} tools registered</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => setShowFilters(!showFilters)}
                            className={`p-2 rounded-lg border transition-all ${showFilters || filterSource ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'bg-white/5 border-white/10 text-zinc-400 hover:text-white'
                                }`}
                        >
                ...
                <div className="relative mb-2">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500/50"
                    />
                </div>
                        ...
                        >
                            <div className="flex flex-wrap gap-1.5 mb-2">
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded text-xs border transition-all ${!filterSource ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-300' : 'bg-white/5 border-white/10 text-zinc-500 hover:text-zinc-300'
                                        }`}
                                >
                                    All ({tools.length})
                                </button>
                                {sources.map(src => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`px-2.5 py-1 rounded text-xs border transition-all flex items-center gap-1.5 ${filterSource === src ? `${cfg.color} bg-white/5 border-white/10` : 'text-zinc-500 bg-transparent border-transparent hover:text-zinc-300'
                                                }`}
                                        >
                                    ...
                                    {sourceTools.map((tool, i) => {
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                className="rounded-lg bg-white/[0.02] border border-white/[0.05] hover:border-white/10 transition-all cursor-pointer"
